    ...
    if (teakTaunt && actualTarget.id !== teakTaunt.id) {
        ...
        log(`[Taunt] Teak ยืนหน้าปกป้อง Phatchee! 💪`, 'text-indigo-400 font-bold');
                ...
                freezeTurns: 0,
                    ...
                    if (getCharStats(actualTarget).hp <= 0) {
                        ...
                        // Altair: บันทึก ATK ของเป้าหมายที่ถูกฆ่า (ใช้สำหรับ On Death immunity)
                        if (attackerEffectiveName === 'Altair') {
                            attacker.altairLastKilledAtk = getCharStats(actualTarget).atk;
                            log(`♛ [Altair] จดจำ ${actualTarget.name} (ATK ${attacker.altairLastKilledAtk}) — จะกลายเป็นภูมิคุ้มกันหากข้าล้มลง`, 'text-yellow-600 font-bold');
                        }
        ...
        function checkDeath(playerKey) {
            ...
            for (let i = p.field.length - 1; i >= 0; i--) {
                ...
                // ── Altair Champion: ภูมิคุ้มกันจาก ATK ของผู้ฆ่าครั้งก่อน ────────
                if (effectiveName === 'Altair' && getCharStats(c).hp <= 0 && typeof playerData !== 'undefined') {
                    const imAtk = playerData.altairImmuneAtk || 0;
                    if (imAtk > 0) {
                        // หาผู้โจมตี (ถ้ามี context)
                        let killerAtk = -1;
                        if (state.lastAttackContext && state.lastAttackContext.attackerPlayerKey) {
                            ...
                            if (killerCard) killerAtk = getCharStats(killerCard).atk;
                        }
                        if (killerAtk === imAtk) {
                            c.hp = 1;
                            c.isDyingProcessing = false;
                            log(`♛ [Altair] ภูมิคุ้มกัน! ข้าจดจำพลังนั้น — รอดชีวิตจาก ATK ${imAtk}!`, 'text-yellow-300 font-bold');
                            continue;
                        }
                    }
                }
...
// ── END BLOCK D ────────────────────────────────────────────────
                ...
                log(`💜 "กลับมาจากความตายอีกแล้ว... Return by Death!"`, 'text-purple-300 font-bold italic');
                    ...
                    // ── Altair Champion: ตายกลับมือ Cost -1 + บันทึกผู้ฆ่าเป็นภูมิคุ้มกัน ──
                    if (effectiveName === 'Altair' && typeof playerData !== 'undefined') {
                        // สะสม cost reduction ถาวร
                        playerData.altairCostBonus = (playerData.altairCostBonus || 0) + 1;
                        const totalBonus = playerData.altairCostBonus;
                        const newEffectiveCost = Math.max(0, 12 - totalBonus);

                        // บันทึก ATK ของผู้ฆ่าเป็นภูมิคุ้มกันครั้งถัดไป
                        let killerName = '???';
                        if (state.lastAttackContext && state.lastAttackContext.attackerPlayerKey) {
                            const killerSide = state.players[state.lastAttackContext.attackerPlayerKey];
                            const killerCard = killerSide ? killerSide.field.find(kc => kc.id === state.lastAttackContext.attackerId) : null;
                            if (killerCard) {
                                playerData.altairImmuneAtk = getCharStats(killerCard).atk;
                                killerName = killerCard.name;
                            }
                        } else if (c.altairLastKilledAtk > 0) {
                            // ตายจาก DoT — ใช้ ATK ของตัวสุดท้ายที่ Altair ฆ่า
                            playerData.altairImmuneAtk = c.altairLastKilledAtk;
                        }
                        saveData();

                        // กลับขึ้นมือพร้อม cost ที่ลดลง
                        p.field.splice(i, 1);
                        c.costReducer = totalBonus;  // effective cost = 12 - totalBonus
                        c.hp = c.maxHp;
                        c.status = [];
                        c.items = [];
                        c.attacksLeft = 0;
                        c.altairLastKilledAtk = 0;
                        c.isDyingProcessing = false;
                        p.hand.push(c);

                        log(`♛ [Altair] "ข้าจะกลับมา... พร้อมแผลเป็นที่เจ็บปวดกว่าเดิม"`, 'text-yellow-200 font-bold italic');
                        log(`♛ [Altair] Return! กลับมาในมือ — Cost เหลือ ${newEffectiveCost} (ลดสะสม ${totalBonus} ครั้ง)`, 'text-yellow-400 font-bold');
                        if (playerData.altairImmuneAtk > 0)
                            log(`♛ [Altair] จดจำ ${killerName} (ATK ${playerData.altairImmuneAtk}) — ภพหน้าจะไม่ล้มต่อสิ่งนี้!`, 'text-orange-300 font-bold');
                        trackKill(gameMode === 'online' ? state.currentTurn : 'player');
                        checkOngoingAuras();
                        continue; // ไม่ push graveyard
                    }
        ...
        function initAuth() {
            ...
            // ── [FIX] ใช้ handleRedirectResult() จาก 07_auth.js แทน
            // เรียกตรงๆ จะทำให้ getRedirectResult ถูก invoke ซ้ำเมื่อ
            // onAuthStateChanged fire หลายรอบ → auth/cancelled-popup-request
            // handleRedirectResult มี _redirectResultChecked guard ป้องกันแล้ว
            if (typeof handleRedirectResult === 'function') {
                handleRedirectResult();
            } else {
                // fallback: เรียกตรงแต่ suppress error ที่ไม่ใช่ของจริง
                auth.getRedirectResult().then(result => {
                    if (result && result.user) {
                        console.log('[Auth] Redirect login success:', result.user.email);
                    }
                }).catch(err => {
                    // auth/cancelled-popup-request = ไม่มี redirect pending → ปกติมาก ไม่ต้องแสดง error
                    if (err.code === 'auth/cancelled-popup-request' ||
                        err.code === 'auth/null-user' ||
                        err.code === 'auth/no-auth-event') return;
