...
if (symbolBlock && actualTarget.id !== symbolBlock.id) {
    ...
    if (teakTaunt && actualTarget.id !== teakTaunt.id) {
        ...
        log(`[Taunt] Teak ยืนหน้าปกป้อง Phatchee! 💪`, 'text-indigo-400 font-bold');
                ...
                // Genie's Lamp: 40% Damage Reduction wish
                if (actualTarget.genieReduction) {
                    dmg = Math.floor(dmg * 0.6);
                    log(`🧞 [Genie's Lamp] 40% Damage Reduction!`, 'text-yellow-300');
                }
                ...
                if (dmg > 0) {
                    ...
                    // ── Altair On Attack: สุ่มชุบเพื่อน 1 ตน ──────────────────
                    if (attackerEffectiveName === 'Altair' && !attacker.silenced && getCharStats(attacker).hp > 0) {
                        const reviveCandidates = atkPlayer.graveyard.filter(c => c.type === 'Character' && !c.status.includes('Poison'));
                        if (reviveCandidates.length > 0 && atkPlayer.field.length < getMaxFieldSlots(state.currentTurn)) {
                            const pick = reviveCandidates[Math.floor(Math.random() * reviveCandidates.length)];
                            const gyIdx = atkPlayer.graveyard.findIndex(c => c.id === pick.id);
                            const revived = atkPlayer.graveyard.splice(gyIdx, 1)[0];
                            revived.hp = revived.maxHp || revived.hp;
                            revived.status = [];
                            revived.attacksLeft = revived.maxAttacks || 1;
                            atkPlayer.field.push(revived);
                            log(`♛ [Altair] On Attack — ดาบแห่งชะตาชุบ ${revived.name} กลับมาจากความตาย!`, 'text-yellow-400 font-bold');
                            triggerOnSummon(revived, state.currentTurn);
                        } else if (reviveCandidates.length === 0) {
                            log(`♛ [Altair] ไม่มีเพื่อนในสุสานให้ชุบ`, 'text-gray-500');
                        } else {
                            log(`♛ [Altair] สนามเต็ม ชุบไม่ได้`, 'text-gray-500');
                        }
                    }
                ...
                freezeTurns: 0,
                    ...
                    // Masayume: ถ้าเป้าหมายยังรอดหลังโดนโจมตี (และโดนดาเมจจริง) → สังหารทันทีแล้วดาบแตก
                    if (!targetIsBase && attacker.items && attacker.items.some(i => i.name === 'Masayume')) {
                        if (dmg > 0 && getCharStats(actualTarget).hp > 0) {
                            log(`⚔️ [Masayume] ดาบแห่งชะตากรรม — สังหารทันที!`, 'text-blue-300 font-bold');
                            actualTarget.hp = -99;
                        }
                        const swordIdx = attacker.items.findIndex(i => i.name === 'Masayume');
                        if (swordIdx !== -1) {
                            const brokenSword = attacker.items.splice(swordIdx, 1)[0];
                            state.players[atkKey].graveyard.push(brokenSword);
                            log(`💔 [Masayume] ดาบแตกถูกส่งลงสุสานหลังใช้งาน`, 'text-gray-400');
                        }
                    }
        ...
        function checkDeath(playerKey) {
            ...
            for (let i = p.field.length - 1; i >= 0; i--) {
                ...
                // ── Altair Champion: ภูมิคุ้มกันการถูกฆ่าจากตัวเดิม ────────
                if (effectiveName === 'Altair' && getCharStats(c).hp <= 0) {
                    const p = state.players[playerKey];
                    if (p.altairImmuneNames && p.altairImmuneNames.length > 0) {
                        let killerName = null;
                        if (state.lastAttackContext && state.lastAttackContext.attackerPlayerKey) {
                            const killerSide = state.players[state.lastAttackContext.attackerPlayerKey];
                            const killerCard = killerSide ? killerSide.field.find(kc => kc.id === state.lastAttackContext.attackerId) : null;
                            if (killerCard) killerName = (killerCard.name.startsWith('Shadow Token') || killerCard.name.startsWith('Shadow army') || killerCard.name.includes('Loki Clone')) ? killerCard.originalName : killerCard.name;
                        }
                        
                        if (killerName && p.altairImmuneNames.includes(killerName)) {
                            c.hp = 1;
                            c.isDyingProcessing = false;
                            log(`♛ [Altair] ภูมิคุ้มกันทำงาน! ข้าเคยพ่ายแพ้ต่อ ${killerName} มาแล้ว... คราวนี้ข้าจะไม่ตาย!`, 'text-yellow-300 font-bold');
                            continue;
                        }
                    }
                }
...
// ── END BLOCK D ────────────────────────────────────────────────
                // Great Rabbit: ตาย → โอกาส 50% เรียกกลับมา 2 ตัว
                if (effectiveName === 'Great Rabbit' && getCharStats(c).hp <= 0) {
                    if (Math.random() < 0.5) {
                        const spawned = [];
                        for (let i = 0; i < 2; i++) {
                            if (p.field.length < getMaxFieldSlots(playerKey)) {
                                const rabbit = createCardInstance('Great Rabbit', 'easter');
                                if (rabbit) { p.field.push(rabbit); spawned.push(rabbit); }
                            }
                        }
                        if (spawned.length > 0) {
                            log(`🐇🐇 [Great Rabbit] ขยายพันธุ์ต่อเนื่อง! เรียก ${spawned.length} Great Rabbit กลับมา!`, 'text-pink-400 font-bold');
                            spawned.forEach(r => triggerOnSummon(r, playerKey));
                        }
                    } else {
                        log(`🐇 [Great Rabbit] ดวงไม่ดี... ไม่ขยายพันธุ์`, 'text-gray-500');
                    }
                }
                ...
                if (effectiveName === 'Luis Pasteur' && getCharStats(c).hp <= 0) {
                    ...
                    if (allies.length > 0) {
                        // Buff: heal 2 random allies (no repeat if only 1 alive)
                        const shuffled = [...allies].sort(() => Math.random() - 0.5);
                        const targets = shuffled.slice(0, Math.min(2, shuffled.length));
                        targets.forEach(target => {
                            target.hp = Math.min(getCharStats(target).maxHp, target.hp + 5);
                            log(`🧫 [Pasteur Death] ${target.name} +5 HP`, 'text-indigo-300 font-bold');
                        });
                    } else {
                        log(`🧫 [Pasteur Death] ไม่มีเพื่อนในสนามให้บัฟ +5 HP`, 'text-gray-500');
                    }
                }
                ...
                log(`💜 "กลับมาจากความตายอีกแล้ว... Return by Death!"`, 'text-purple-300 font-bold italic');
                    ...
                    // ── Altair Champion: ตายกลับมือ ลด Cost + บันทึกชื่อผู้ฆ่า ──
                    if (effectiveName === 'Altair') {
                        const p = state.players[playerKey];
                        if (!p.altairCostBonus) p.altairCostBonus = 0;
                        if (!p.altairImmuneNames) p.altairImmuneNames =[];

                        p.altairCostBonus += 1;
                        const newEffectiveCost = Math.max(0, 12 - p.altairCostBonus);

                        // บันทึกชื่อของการ์ดที่ฆ่า
                        let killerName = '???';
                        if (state.lastAttackContext && state.lastAttackContext.attackerPlayerKey) {
                            const killerSide = state.players[state.lastAttackContext.attackerPlayerKey];
                            const killerCard = killerSide ? killerSide.field.find(kc => kc.id === state.lastAttackContext.attackerId) : null;
                            if (killerCard) {
                                killerName = (killerCard.name.startsWith('Shadow Token') || killerCard.name.startsWith('Shadow army') || killerCard.name.includes('Loki Clone')) ? killerCard.originalName : killerCard.name;
                                if (!p.altairImmuneNames.includes(killerName)) {
                                    p.altairImmuneNames.push(killerName);
                                }
                            }
                        }

                        // กลับขึ้นมือพร้อม cost ที่ลดลง
                        p.field.splice(i, 1);
                        c.costReducer = p.altairCostBonus;
                        c.hp = c.maxHp;
                        c.status = [];
                        c.items =[];
                        c.attacksLeft = 0;
                        ...
                        log(`♛ [Altair] "ความตายเป็นเพียงบทเรียน..."`, 'text-yellow-200 font-bold italic');
                        log(`♛ [Altair] Return! กลับมาในมือ — Cost เหลือ ${newEffectiveCost} (ลดสะสมในเกมนี้ ${p.altairCostBonus} ครั้ง)`, 'text-yellow-400 font-bold');
                        
                        if (killerName !== '???') {
                            log(`♛ [Altair] จดจำพลังของ ${killerName} — ภพหน้าเจ้าจะไม่มีทางสังหารข้าได้อีก!`, 'text-orange-300 font-bold');
                        }
                        
                        trackKill(gameMode === 'online' ? state.currentTurn : 'player');
                        checkOngoingAuras();
                        continue; // ไม่ push ลง graveyard
                    }
        ...
        function initAuth() {
            ...
            // ── [FIX] getRedirectResult: รองรับ fallback redirect จาก googleLogin()
            // เรียกแค่ครั้งเดียว + suppress auth/cancelled-popup-request
            // (error นี้เกิดเมื่อโหลดหน้าปกติที่ไม่ได้มาจาก redirect — ไม่ใช่ error จริง)
            auth.getRedirectResult().then(result => {
                if (result && result.user) {
                    console.log('[Auth] Redirect login success:', result.user.email);
                    // onAuthStateChanged จะ fire ต่อเองอัตโนมัติ
                }
            }).catch(err => {
                if (err.code === 'auth/cancelled-popup-request' ||
                    err.code === 'auth/null-user' ||
                    err.code === 'auth/no-auth-event') {
                    // โหลดหน้าปกติ ไม่มี redirect pending — ไม่ต้องทำอะไร
                    return;
                }
        ...
        // ── [FIX] loginWithGoogle ใช้ signInWithRedirect แทน Popup (รองรับ Mobile) ──
