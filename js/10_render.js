        ...
        function renderField(containerId, cards, owner) {
            ...
            cards.forEach(c => {
                ...
                if (state.targeting.active) {
                    ...
                    const canControl = isOnlineOrDraft ? (owner === myRole && state.currentTurn === myRole) : (state.currentTurn === owner);
                    ...
                    if (canControl && state.phase === 'BATTLE') {
                        if (c.attacksLeft > 0 && getCharStats(c).hp > 0) {
                            ...
                            // [FIX] ป้องกัน player คลิกการ์ด AI ในโหมด AI
                            const isPlayerControllable = gameMode !== 'ai' || owner === 'player';
