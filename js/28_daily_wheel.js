...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof COSMETICS_CATALOG !== 'undefined') {
        if (!COSMETICS_CATALOG.avatars.find(a => a.id === 'av_nobita')) {
            COSMETICS_CATALOG.avatars.push({ id: 'av_nobita', label: "Nobita's Bright Smile", art: 'https://copilot.microsoft.com/th/id/BCO.c5616167-ad5a-4d1f-93b5-a7839328131e.png', locked: true });
        }
        if (!COSMETICS_CATALOG.banners.find(b => b.id === 'bn_nobita')) {
            COSMETICS_CATALOG.banners.push({ id: 'bn_nobita', label: "Nobita's Dream Odyssey", art: 'https://copilot.microsoft.com/th/id/BCO.73f22404-bc70-4e59-b121-13f1a1313ccc.png', locked: true });
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.buyUltimateArtstyle = function(id) {
        ...
        if (cfg.isBPExclusive) {
            if (typeof showToast === 'function') showToast('❌ สกินนี้หาได้จากกล่อง JJK Box ใน Battle Pass เท่านั้น!', '#f87171');
            return;
        } else if (cfg.currency === 'wheel') {
    ...
    window.renderUltimateArtstyleShop = function() {
        ...
        const rows = Object.values(allStyles).map(cfg => {
            ...
            if (cfg.isBPExclusive) {
                canBuy = false;
                costText = `<span style="color:#fbbf24">📦 JJK Box เท่านั้น</span>`;
                btnLabel = 'เปิดกล่อง 📦';
                btnColor = 'linear-gradient(135deg,#7f1d1d,#450a0a)';
            } else if (cfg.currency === 'wheel') {
                canBuy = false; 
            ...
            <div style="background:#0f172a;border:1.5px solid ${borderClr};
              ...
              <div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0">
                ${isUnlocked
                  ...
                  : (cfg.isBPExclusive
                    ? `<button onclick="showHubTab('bp');document.getElementById('_artstyle-overlay').remove()" style="background:${btnColor};color:white;border:none;padding:7px 10px;border-radius:8px;font-size:0.75rem;font-weight:700;cursor:pointer;min-width:64px">${btnLabel}</button>`
                  : (cfg.currency === 'wheel'
...
// ============================================================
// HOTFIX: Fix Battle Pass Titles and Artstyles (ย้ายมาใส่รวมในนี้)
// ============================================================

document.addEventListener('DOMContentLoaded', () => {

    // 1. เพิ่มฉายา (Titles) จาก Battle Pass เข้าไปในสารบบของเกมให้แสดงผลได้
    if (typeof ALL_TITLES !== 'undefined') {
        if (!ALL_TITLES.find(t => t.id === 'The Honored One')) {
            ALL_TITLES.push({ id: 'The Honored One', how: 'รับจาก Battle Pass: JJK (Free Lv.50)' });
        }
        if (!ALL_TITLES.find(t => t.id === 'Malevolent Cursed King')) {
            ALL_TITLES.push({ id: 'Malevolent Cursed King', how: 'รับจาก Battle Pass: JJK (Premium Lv.50)' });
        }
    }

    // 2. แก้บัคการเพิ่มสกิน (Artstyle) ลงในร้านค้าให้ถูกต้อง
    if (typeof ARTSTYLE_CFG !== 'undefined') {
        Object.assign(ARTSTYLE_CFG, {
            'megumi_jp': { id:'megumi_jp', label:'Megumi - JP Art', emoji:'🖌️', targetCard:'Megumi Fushiguro', art:'https://file.garden/aeeLCXSsJxTPrRbp/enhanced_image_soft.png', shopCost:0, isBPExclusive: true },
            'toji_wrath': { id:'toji_wrath', label:'Toji - Cursed Wrath', emoji:'🗡️', targetCard:'Toji Fushiguro', art:'https://file.garden/aeeLCXSsJxTPrRbp/1777121615565.jpg', shopCost:0, isBPExclusive: true },
            'mahoraga_mafia': { id:'mahoraga_mafia', label:'Mahoraga - Mafia', emoji:'🛞', targetCard:'Mahoraga', art:'https://file.garden/aeeLCXSsJxTPrRbp/396d790266037a28bae263dea7dbe086.jpg', shopCost:0, isBPExclusive: true }
        });
    }

    // 3. รันสคริปต์ซ่อมแซมย้อนหลังให้คนที่เคยกด Claim ไปแล้วแต่ของไม่เข้า
    function runGlobalBPFix() {
        ...
        allClaimed.forEach(reward => {
            ...
            const checkReward = (r) => {
                ...
                // ซ่อมฉายาที่หายไป
                if (r && r.type === 'title' && !playerData.unlockedTitles.includes(r.title)) {
                    playerData.unlockedTitles.push(r.title);
                    changed = true;
                }
            };
            
            if (reward.type === 'multi') {
                reward.items?.forEach(checkReward);
            } else {
                checkReward(reward);
            }
        });
