...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --------------------------------------------------------
    // FIX 1: ซ่อมบัค Firebase เซฟการ์ดที่มีจุด (.) เช่น H.P. Lovecraft ไม่ได้
    // --------------------------------------------------------
    if (typeof window.saveData === 'function') {
        window.saveData = function() {
            ...
            for (const k in playerData.collection) {
                ...
                safeCollection[safeKey] = playerData.collection[k];
            }
            const originalCollection = playerData.collection;
            playerData.collection = safeCollection;
            ...
            playerData.collection = originalCollection;
        };
    }

    // ถอดรหัสตอนโหลดจาก Firebase
