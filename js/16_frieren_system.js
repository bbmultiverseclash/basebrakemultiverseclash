...
;(function _frierenInit(){
    ...
    if(typeof renderPacksPanel==='function'){
        ...
        window.renderPacksPanel=function(){
            _o();
            // Small delay to ensure innerHTML is set before we append
            setTimeout(_appendFrierenSection, 0);
        };
    }
    ...
    // Re-render packs panel so Frieren section appears immediately
    setTimeout(()=>{ if(typeof renderPacksPanel==='function') renderPacksPanel(); },80);
})();
...
function _appendFrierenSection(){
    ...
    sec.id='_frieren-gem-sec'; sec.style.cssText='padding:8px 16px 16px';
...
<div style="background:linear-gradient(135deg,#0d0b2e,#1a1247);border:2.5px solid ${available?'#818cf8':'#374151'};border-radius:20px;overflow:hidden">
  ...
  <div style="padding:14px">
    ...
    // Append directly to panel — most reliable approach
