...
function executeRemoteAction(action) {
    ...
    if (action.type === 'nextPhase') {
        // ใช้ fromPhase จาก P2 เพื่อป้องกัน state drift
        // ถ้า fromPhase ไม่ตรงกับ state ปัจจุบัน → ไม่ execute
        if (action.fromPhase && action.fromPhase !== state.phase) return;
