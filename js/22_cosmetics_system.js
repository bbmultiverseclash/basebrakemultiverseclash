...
function renderCosmeticSection(title, type) {
    ...
    items.forEach(item => {
        ...
        const preview = type === 'avatars' ? `<img src="${item.art}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">`
            ...
            : item.id !== 'fr_none' ? `<div style="width:35px;height:35px;border-radius:50%;border:3px solid ${item.color};box-shadow:0 0 8px ${item.color};"></div>`
    ...
    if (emojiSlot) {
        ...
        emojiSlot.innerHTML = `
            <div style="position:relative;width:80px;height:80px;border-radius:50%;">
                <img src="${imgUrl}"
                     class="${frameEffect}"
                     style="width:100%;height:100%;border-radius:50%;object-fit:cover;border:4px solid ${frameColor || '#111827'};color:${frameColor || 'transparent'};">
            </div>`;
