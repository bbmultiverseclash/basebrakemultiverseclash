...
// ดึง avatar/banner จาก BP ที่ Claim แล้วมาปลดล็อกใน unlockedCosmetics อัตโนมัติ
function syncBPCosmetics() {
    ...
    // สาย Free
    (playerData.bpClaimedFree || []).forEach(lv => {
        const reward = BP_REWARDS.free?.[lv];
        if (reward && (reward.type === 'avatar' || reward.type === 'banner')) {
            if (!playerData.unlockedCosmetics.includes(reward.id)) {
                playerData.unlockedCosmetics.push(reward.id);
                console.log('Auto-unlocked from Free BP:', reward.id);
            }
        }
    });

    // สาย Premium (รองรับทั้ง item เดี่ยว และ multi-reward เช่น Yuta)
    (playerData.bpClaimedPremium || []).forEach(lv => {
        const reward = BP_REWARDS.premium?.[lv];
        if (!reward) return;

        if (reward.type === 'multi') {
            (reward.items || []).forEach(sub => {
                if ((sub.type === 'avatar' || sub.type === 'banner') &&
                    !playerData.unlockedCosmetics.includes(sub.id)) {
                    playerData.unlockedCosmetics.push(sub.id);
                    console.log('Auto-unlocked from Premium BP (multi):', sub.id);
                }
            });
        } else if (reward.type === 'avatar' || reward.type === 'banner') {
            if (!playerData.unlockedCosmetics.includes(reward.id)) {
                playerData.unlockedCosmetics.push(reward.id);
                console.log('Auto-unlocked from Premium BP:', reward.id);
            }
        }
    });
...
function renderProfilePanel() {
    syncBPCosmetics(); // ✅ ตรวจสอบ BP rewards ก่อนวาดหน้า
