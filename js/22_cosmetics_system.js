...
const COSMETICS_CATALOG = {
    avatars: [
        ...
        // --- ของรางวัลจาก Battle Pass ---
        { id: 'av_yuji',    label: 'Yuji Itadori',     art: 'https://file.garden/aeeLCXSsJxTPrRbp/7a3ffe40ffa26b52a3652d939fd8273f.jpg',   locked: true },
        { id: 'av_yuta',    label: 'Yuta Okkotsu',     art: 'https://file.garden/aeeLCXSsJxTPrRbp/d7ae8acce56e71262672568eff870699.jpg',   locked: true },
        { id: 'av_hiromi',  label: 'Hiromi Higuruma',  art: 'https://file.garden/aewtACVxx2XDWiFH/95496.jpg',                              locked: true },
        { id: 'av_kenjaku', label: 'Kenjaku',          art: 'https://file.garden/aeeLCXSsJxTPrRbp/b6595a016c7a33c05ef0b24922a492e7.jpg',   locked: true }
    ],
    banners: [
        ...
        // --- ของรางวัลจาก Battle Pass ---
        { id: 'bn_today',       label: 'Strongest Today',   art: 'https://i.ibb.co/GQRYm9vw/image.png',                                       locked: true },
        { id: 'bn_history',     label: 'Strongest History', art: 'https://i.ibb.co/4nW1WZNS/image.png',                                       locked: true },
        { id: 'bn_ms_potential',label: 'MS Potential',      art: 'https://file.garden/aeeLCXSsJxTPrRbp/1777097003700.jpg',                    locked: true },
        { id: 'bn_sukuna_gojo', label: 'Sukuna vs Gojo',    art: 'https://file.garden/aeeLCXSsJxTPrRbp/1777102946721.jpg',                    locked: true }
    ],
    frames: [
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.renderHomePanel = function() {
        if (_origRenderHome) _origRenderHome.apply(this, arguments);
        setTimeout(applyCosmeticsToHome, 50); // รอ DOM render ก่อน
    };
    const _origShowHubTab = window.showHubTab;
    window.showHubTab = function(tab) {
        if (tab === 'profile') {
            // Hide others
            ['home', 'packs', 'collection', 'deckbuilder', 'play', 'bp'].forEach(t => {
...
// --- SYNC BATTLE PASS COSMETICS ---
function syncBPCosmetics() {
    if (typeof playerData === 'undefined') return;
    if (!playerData.unlockedCosmetics) playerData.unlockedCosmetics = [];

    // ถ้ามี BP_REWARDS จาก 23_battle_pass.js ใช้ระบบ generic แทน hardcode
    if (typeof BP_REWARDS !== 'undefined') {
        const allClaimed = {
            free:    new Set(playerData.bpClaimedFree    || []),
            premium: new Set(playerData.bpClaimedPremium || [])
        };
        Object.entries(BP_REWARDS).forEach(([pool, levels]) => {
            Object.entries(levels).forEach(([lv, reward]) => {
                if (!allClaimed[pool].has(parseInt(lv))) return;
                const process = (r) => {
                    if ((r.type === 'avatar' || r.type === 'banner' || r.type === 'frame') &&
                        !playerData.unlockedCosmetics.includes(r.id)) {
                        playerData.unlockedCosmetics.push(r.id);
                    }
                };
                if (reward.type === 'multi') reward.items.forEach(process);
                else process(reward);
            });
        });
    } else {
        // fallback: hardcode สำหรับกรณี BP_REWARDS ยังโหลดไม่ทัน
        const freeMap  = { 5: 'av_yuji', 15: 'bn_today', 30: 'av_hiromi', 40: 'bn_ms_potential' };
        const premMap  = { 1: 'av_yuta', 15: 'bn_history', 30: 'av_kenjaku', 40: 'bn_sukuna_gojo' };
        (playerData.bpClaimedFree    || []).forEach(lv => { const id = freeMap[lv]; if (id && !playerData.unlockedCosmetics.includes(id)) playerData.unlockedCosmetics.push(id); });
        (playerData.bpClaimedPremium || []).forEach(lv => { const id = premMap[lv]; if (id && !playerData.unlockedCosmetics.includes(id)) playerData.unlockedCosmetics.push(id); });
    }
    saveData();
}
        ...
        <div class="profile-card" style="margin-bottom: 20px;">
            ...
            <div style="padding: 0 25px 25px; margin-top: -60px; position: relative; text-align: center;">
                ...
                <div style="width: 120px; height: 120px; margin: 0 auto 15px; position: relative;">
                    <img src="${avatar}" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover; background: #000; ${frame?.art ? '' : `border: 4px solid #111827;`}" 
                         class="${(!frame?.art && frame?.effect !== 'none') ? 'frame-' + frame?.effect : ''}" style="color: ${frame?.color}">
                    ${frame?.art ? `<img src="${frame.art}" style="position: absolute; inset: -18px; width: calc(100% + 36px); height: calc(100% + 36px); object-fit: contain; pointer-events: none; z-index: 10;">` : ''}
                </div>
...
function renderCosmeticSection(title, type) {
    ...
    items.forEach(item => {
        const isUnlocked = !item.locked || (playerData.unlockedCosmetics && playerData.unlockedCosmetics.includes(item.id));
        const isEq = isUnlocked && (
            (type === 'avatars'  && playerData.equippedAvatar === item.id) ||
            (type === 'banners'  && playerData.equippedBanner === item.id) ||
            (type === 'frames'   && playerData.equippedFrame  === item.id)
        );

        const preview = type === 'avatars' ? `<img src="${item.art}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">`
            : type === 'banners' ? `<img src="${item.art}" style="width:100%;height:60%;object-fit:cover;border-radius:8px;">`
            : item.id !== 'fr_none' ? (item.art 
                ? `<div style="position:relative;width:35px;height:35px;border-radius:50%;background:#111827;"><img src="${item.art}" style="position:absolute;inset:-6px;width:calc(100% + 12px);height:calc(100% + 12px);object-fit:contain;"></div>` 
                : `<div style="width:35px;height:35px;border-radius:50%;border:3px solid ${item.color};box-shadow:0 0 8px ${item.color};"></div>`)
            : `<div style="color:#6b7280;font-size:1.5rem;">🚫</div>`;

        if (!isUnlocked) {
            // แสดง preview พร้อม overlay 🔒 — ไม่สามารถคลิกได้
            html += `
            <div class="cosmetic-box" style="opacity:0.45;cursor:not-allowed;position:relative;" title="🔒 ได้จาก Battle Pass">
                <div style="width:100%;aspect-ratio:1;background:#111827;border-radius:8px;overflow:hidden;display:flex;align-items:center;justify-content:center;margin-bottom:5px;position:relative;">
                    ${preview}
                    <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.45);border-radius:8px;font-size:1.2rem;">🔒</div>
                </div>
                <div style="font-size:0.55rem;color:#6b7280;font-weight:bold;">${item.label}</div>
                <div style="font-size:0.45rem;color:#4b5563;">Battle Pass</div>
            </div>`;
        } else {
            html += `
            <div class="cosmetic-box ${isEq ? 'active' : ''}" onclick="setCosmetic('${type}', '${item.id}')">
                ...
                ${isEq ? `<div style="font-size:0.45rem;color:#4ade80;">✅ Equipped</div>` : ''}
            </div>`;
        }
    });
...
// --- APPLY COSMETICS TO HOME PANEL ---
function applyCosmeticsToHome() {
    // แปะ Banner บน hub-card
    const rankCard = document.querySelector('.hub-card');
    if (rankCard && playerData.equippedBanner) {
        const bannerData = COSMETICS_CATALOG.banners.find(b => b.id === playerData.equippedBanner);
        if (bannerData?.art) {
            rankCard.style.backgroundImage = `linear-gradient(to right, rgba(0,0,0,0.82), rgba(0,0,0,0.35)), url('${bannerData.art}')`;
            rankCard.style.backgroundSize = 'cover';
            rankCard.style.backgroundPosition = 'center';
        }
    }

    // แทนที่ emoji avatar ด้วยรูปจริง + กรอบ
    const emojiSlot = rankCard?.querySelector('div[style*="font-size:3.5rem"], div[style*="font-size: 3.5rem"]');
    if (emojiSlot) {
        const avData  = COSMETICS_CATALOG.avatars.find(a => a.id === playerData.equippedAvatar);
        const frData  = COSMETICS_CATALOG.frames.find(f => f.id === playerData.equippedFrame);
        const imgUrl  = avData?.art || 'https://www.gstatic.com/images/branding/product/2x/avatar_anonymous_96x96dp.png';
        const frameColor  = (frData && frData.id !== 'fr_none') ? frData.color : '';
        const frameEffect = (frData && frData.effect !== 'none') ? `frame-${frData.effect}` : '';

        emojiSlot.innerHTML = `
            <div style="position:relative;width:80px;height:80px;border-radius:50%;">
                <img src="${imgUrl}"
                     class="${!frData?.art ? frameEffect : ''}"
                     style="width:100%;height:100%;border-radius:50%;object-fit:cover; ${!frData?.art ? `border:4px solid ${frameColor || '#111827'};color:${frameColor || 'transparent'};` : ''}">
                ${frData?.art ? `<img src="${frData.art}" style="position: absolute; inset: -12px; width: calc(100% + 24px); height: calc(100% + 24px); object-fit: contain; pointer-events: none; z-index: 10;">` : ''}
            </div>`;
        emojiSlot.style.fontSize = '0';
    }
}
