...
const RZ_POOL = [
    'Beatrice', 'Sirius', 'Echidna', 'Garfiel Tinsel', 'Pandora',
    'Regulus Corneas', 'Petelgeuse Romanée-Conti', 'Otto Suwen',
    'Cecilus Segmunt', 'Satella'
];

// ─── Reveal overlay ──────────────────────────────────────────────────────────
function _showRezeroReveal(cardName) {
    if (typeof CardSets === 'undefined') return;
    const card = CardSets['isekai_adventure']?.[cardName];
    const art = card?.art || '';
    const ov = document.createElement('div');
    ...
    ov.innerHTML = `
        <div style="background:linear-gradient(135deg,#2e1065,#1e1b4b);border:3px solid #a855f7;border-radius:24px;padding:28px;max-width:340px;width:90%;text-align:center;">
            ...
            <div style="color:white;font-size:1.1rem;font-weight:900;margin-bottom:16px;">${cardName}</div>
...
window.pullRezeroPack = function() {
    ...
    if ((playerData.coins || 0) < 1500) {
        if (typeof showToast === 'function') showToast('🪙 ทองไม่พอ! ต้องการ 1,500 🪙', '#f87171');
        return;
    }
    if ((playerData.rzPacksBought || 0) >= 10) {
...
// ─── Inject Re:Zero Pack UI into renderPacksPanel ────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
    const _origRenderPacksRZ = window.renderPacksPanel;
    if (typeof _origRenderPacksRZ === 'function') {
        window.renderPacksPanel = function() {
            _origRenderPacksRZ.call(this);
            if (new Date() > RZ_PACK_EXPIRY) return;
            const panel = document.getElementById('hub-panel-packs');
            if (!panel) return;
            if (panel.querySelector('#rz-limited-pack-banner')) return; // กันซ้ำ
            const bought = (typeof playerData !== 'undefined' ? playerData.rzPacksBought : 0) || 0;
            const remaining = 10 - bought;
            const div = document.createElement('div');
            div.id = 'rz-limited-pack-banner';
            div.innerHTML = `
                <div style="margin:16px 0 20px;background:linear-gradient(135deg,#2e1065,#1e1b4b);border:2.5px solid #a855f7;border-radius:20px;padding:16px;text-align:center;">
                    <div style="color:#e879f9;font-weight:900;font-size:1rem;margin-bottom:4px;">🔮 RE:ZERO LIMITED PACK</div>
                    <div style="color:#c084fc;font-size:0.78rem;margin-bottom:10px;">สุ่มการ์ด Re:Zero 1 ใบ (${remaining}/10 ครั้งคงเหลือ) | หมดเขต 3 พ.ค. 2026</div>
                    <button onclick="pullRezeroPack()" ${remaining <= 0 ? 'disabled' : ''} style="background:${remaining > 0 ? '#a855f7' : '#4b5563'};color:white;border:none;padding:10px 24px;border-radius:10px;font-weight:bold;font-size:0.9rem;cursor:${remaining > 0 ? 'pointer' : 'not-allowed'};">เปิดสุ่ม (1,500 🪙)</button>
                </div>`;
