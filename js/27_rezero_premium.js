...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const REZERO_EVOLVED = {
        ...
        // Spells for Ram
        'El Fula': { name: 'El Fula', type: 'Spell', cost: 3, text: 'Ram ของคุณเสีย 4 HP เพื่อทำ 10 ดาเมจใส่ศัตรูสุ่ม 1 ตัว (ใช้ไม่ได้หากไม่มี Ram)', color: 'bg-pink-400', requiresTarget: false, _theme: 'isekai_adventure' },
        'Bloodline Link': { name: 'Bloodline Link', type: 'Spell', cost: 5, text: 'ทำดาเมจใส่ศัตรูทั้งหมด เท่ากับ ATK รวมของ Ram และ Rem บนสนามของคุณ', color: 'bg-rose-500', requiresTarget: false, _theme: 'isekai_adventure' },
        'Hura': { name: 'Hura', type: 'Spell', cost: 3, text: 'ทำ 2 ดาเมจใส่ศัตรูสุ่ม 1 ตัว หากเป้าหมายไม่ตาย จะเด้งกลับขึ้นมือศัตรู และ Cost +5', color: 'bg-pink-300', requiresTarget: false, _theme: 'isekai_adventure' }
    };
    ...
    if (typeof ENHANCE_RECIPES !== 'undefined') {
        Object.assign(ENHANCE_RECIPES, {
            'Subaru v2': {
                ...
                gems: 0, coins: 0, rezeroTokens: 25, result: 'Subaru v2|isekai_adventure'
            },
            'Ram (The Awakened Oni Goddess)': {
                baseCard: 'Ram|isekai_adventure',
                materials: [{ name: 'Ram', theme: 'isekai_adventure', count: 9 }, { name: 'Rem', theme: 'isekai_adventure', count: 10 }],
                gems: 0, coins: 2200, rezeroTokens: 22, result: 'Ram (The Awakened Oni Goddess)|isekai_adventure'
            },
            'Reinhard (Full Power)': {
                baseCard: 'Reinhard|isekai_adventure',
                materials: [{ name: 'Reinhard', theme: 'isekai_adventure', count: 4 }],
                gems: 10, coins: 3000, rezeroTokens: 40, result: 'Reinhard (Full Power)|isekai_adventure'
            }
        });
    }

    // 3. ทับระบบ Enhance UI ให้รองรับ Re:Zero Tokens
    if (typeof window.openEnhanceModal === 'function') {
        window.openEnhanceModal = function(recipeKey) {
            ...
            const hasRZ = recipe.rezeroTokens ? ((playerData.rezeroTokens || 0) >= recipe.rezeroTokens) : true;
            const canDo = (ownedBase >= 1 && hasCoins && hasGems && hasRZ && hasMats);
            ...
            ov.innerHTML = `
                <div style="background:linear-gradient(135deg,#1e1b4b,#0f172a);border:3px solid #7c3aed;border-radius:24px;padding:24px;max-width:380px;width:90%;text-align:center;">
                    ...
                    <div style="background:rgba(0,0,0,0.3);padding:10px;border-radius:10px;margin-bottom:10px;text-align:left;font-size:0.8rem">
                        ...
                        ${recipe.rezeroTokens > 0 ? `<div style="color:${hasRZ ? '#c084fc' : '#f87171'}">🔮 Re:Zero Tokens: ${(playerData.rezeroTokens || 0)}/${recipe.rezeroTokens}</div>` : ''}
                    </div>
        ...
        window.confirmEnhance = function(recipeKey) {
            ...
            if (recipe.rezeroTokens) playerData.rezeroTokens -= recipe.rezeroTokens;
