    ...
    if (typeof window.initiateAttack === 'function') {
        ...
        window.initiateAttack = function(attackerId, targetId, targetIsBase) {
            ...
            if (attacker && target && typeof getCharStats !== 'undefined') {
                ...
                // [FIX] หลอกระบบไม่ให้เพื่อนตาย โดยเพิ่ม HP 10,000 ก่อนโดนดาเมจ
                if (volcanica) {
                    target.hp += 10000;
                }

                // ปล่อยให้ระบบโจมตีทำงานตามปกติ
                _origAttack.apply(this, arguments);

                // คืนค่าพลังโจมตีให้ Reid
                if (attacker._reidBoosted) {
                    attacker.atk = Math.round(attacker.atk / 3);
                    attacker._reidBoosted = false;
                }

                // ── ระบบรับดาเมจแทนของ Volcanica ──
                if (volcanica) {
                    // คำนวณดาเมจที่โดนจริงๆ
                    let rawDamage = (startHp + 10000) - target.hp;
                    
                    // กรณีโดนท่า Instant Kill (เช่น Mike Tyson สั่ง HP = -99)
                    if (target.hp <= -99) {
                        rawDamage = target.maxHp; 
                    }

                    // คืนค่า HP เดิมให้เป้าหมาย (เป้าหมายไม่ได้รับดาเมจเลย 100%)
                    target.hp = startHp;
                    
                    // Volcanica รับดาเมจ 20%
                    if (rawDamage > 0) {
                        const redirected = Math.max(1, Math.floor(rawDamage * 0.2));
                    ...
                    // อัปเดต UI เพื่อให้หลอดเลือดแสดงค่าถูกต้องทันที
