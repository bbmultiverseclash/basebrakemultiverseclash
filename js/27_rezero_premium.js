    ...
    window.startDungeon_v2 = function(dungeonId) {
        ...
        window.buildDeck = function(theme) {
            if (theme === 'dungeon_boss') {
                ...
                // 20 White Whale (Cost 5, +3 stat)
                for (let i = 0; i < 20; i++) bossDeck.push(makeBossCard('White Whale', 'isekai_adventure', 5, 3));
                // 15 Great Rabbit (Cost 2, +2 stat)
                for (let i = 0; i < 15; i++) bossDeck.push(makeBossCard('Great Rabbit', 'easter', 2, 2));
                // 15 Black Serpent (Cost 3, +3 stat)
                for (let i = 0; i < 15; i++) bossDeck.push(makeBossCard('Black Serpent', 'isekai_adventure', 3, 3));
                // 5 Daphne (Cost เดิม, +7 stat)
                for (let i = 0; i < 5; i++) bossDeck.push(makeBossCard('Daphne', 'isekai_adventure', null, 7));
                // 10 Wild Kingdom (Cost 8 ปกติ)
                for (let i = 0; i < 10; i++) bossDeck.push(makeBossCard('Wild Kingdom', 'animal_kingdom', 8, 0));
        ...
        setTimeout(() => {
            ...
            if (boss) {
                boss.cost = 5;
                boss.atk += 3; boss.hp += 3; boss.maxHp += 3;
                boss.attacksLeft = 1;
