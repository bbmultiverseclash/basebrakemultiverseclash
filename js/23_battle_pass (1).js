...
const QUEST_DEFS = {
    daily: [
        ...
        { id: 'daily_epic_pack',  label: 'Open 1 Epic Pack',     desc: 'Open at least 1 Epic pack.',             icon: '📦', target: 1,    reward: 800,  trackKey: 'epicPacksOpened' },
    ],
    weekly: [
...
// ─── Promo Codes ──────────────────────────────────────────────
const PROMO_CODES = {
    'FIRSTBP': { reward: 10000, rewardType: 'bp', label: '+10,000 BP', maxRedeems: 1, desc: 'First Battle Pass promo code!' },
};
...
window.QuestSystem = {
    ...
    trackEpicPackOpened() {
        _initQuestData();
        playerData.quests.dailyProgress['epicPacksOpened'] = (playerData.quests.dailyProgress['epicPacksOpened'] || 0) + 1;
        if (typeof saveData === 'function') saveData();
    },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Patch awardWin: rank win + AI game tracking
    if (typeof window.awardWin === 'function') {
        const _ow = window.awardWin;
        window.awardWin = function() {
            const r = _ow.apply(this, arguments);
            if (typeof gameMode !== 'undefined') {
                if (gameMode === 'online') window.QuestSystem.trackRankWin();
                if (gameMode === 'ai')     window.QuestSystem.trackGameVsAI();
            }
            return r;
        };
    }
    // Patch awardLoss: AI game tracking
    if (typeof window.awardLoss === 'function') {
        const _ol = window.awardLoss;
        window.awardLoss = function() {
            const r = _ol.apply(this, arguments);
            if (typeof gameMode !== 'undefined' && gameMode === 'ai')
                window.QuestSystem.trackGameVsAI();
            return r;
        };
    }
    // Patch trackDamageBase: feed weekly base-damage quest
    if (typeof window.trackDamageBase === 'function') {
        ...
        window.trackDamageBase = function(dmg, forPlayer) {
            ...
            const myKey = (typeof myRole !== 'undefined') ? myRole : 'player';
            if ((forPlayer || myKey) === myKey) window.QuestSystem.trackBaseDamage(dmg);
        };
    }

    // Patch showHubTab to handle 'quests' tab
    if (typeof window.showHubTab === 'function') {
        const _ot = window.showHubTab;
        window.showHubTab = function(tab) {
            if (tab === 'quests') {
                document.querySelectorAll('.hub-tab-btn').forEach(b => b.classList.remove('active-tab'));
...
function _injectQuestTab() {
    const tabBar = document.querySelector('.hub-tabs') || document.getElementById('hub-tabs');
    if (tabBar && !document.getElementById('quest-tab-btn')) {
        const btn = document.createElement('button');
        btn.id = 'quest-tab-btn';
        btn.className = 'hub-tab-btn';
        btn.textContent = '📋 Quests';
        btn.onclick = () => { if (typeof showHubTab === 'function') showHubTab('quests'); };
        tabBar.appendChild(btn);
    }
    const hub = document.getElementById('hub-content') || document.getElementById('hub-panels');
    if (hub && !document.getElementById('hub-panel-quests')) {
        const pnl = document.createElement('div');
...
function renderQuestPanel() {
    ...
    const renderSection = (title, type, defs, progressMap, claimedMap) => defs.map(def => {
    ...
    <div style="max-width:500px;margin:0 auto;padding:20px;">
        ...
        <div style="margin-bottom:20px;">
            ...
            ${renderSection('daily',   'daily',   QUEST_DEFS.daily,   q.dailyProgress  || {}, q.dailyClaimed  || {})}
        </div>
        <div style="margin-bottom:20px;">
            ...
            ${renderSection('weekly',  'weekly',  QUEST_DEFS.weekly,  q.weeklyProgress || {}, q.weeklyClaimed || {})}
        </div>
        <div style="margin-bottom:20px;">
            ...
            ${renderSection('oneTime', 'oneTime', QUEST_DEFS.oneTime, q.otProgress     || {}, q.otClaimed     || {})}
        </div>
        <div style="margin-bottom:20px;">
