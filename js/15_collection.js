...
// การ์ดที่ไม่สามารถสุ่มออกจาก Pack ได้ (ต้องได้รับจากระบบพิเศษเท่านั้น)
...
function getSetCardPool(setName) {
    ...
    return Object.entries(cards)
        .filter(([name, data]) => !data.isChampion && !PACK_EXCLUDED.has(name))  // ไม่รวม Champion และการ์ด Frieren System
        .map(([name, data]) => ({
    ...
    if (typeof CardSets !== 'undefined') {
        Object.entries(CardSets).forEach(([setName, cards]) => {
            ...
            Object.entries(cards).forEach(([name, data]) => {
                if (!data.isChampion && !PACK_EXCLUDED.has(name)) {
