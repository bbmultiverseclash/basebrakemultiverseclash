...
function _applyRedeemReward(reward) {
    ...
    if (reward.rp)                                 playerData.rp            = (playerData.rp || 0) + reward.rp;
    if (reward.xp)                                 playerData.xp            = (playerData.xp || 0) + reward.xp;
    if (reward.wins)                               playerData.wins          = (playerData.wins || 0) + reward.wins;
    if (reward.losses != null && reward.losses === 0) playerData.losses     = 0;
    saveData();
    updateHubUI();
}
    ...
    if (reward.serverClaim) {
        ...
        try {
            const result = await claimRef.transaction(current => {
                if (current !== null) return; // already claimed — abort transaction
            ...
            // We won the race — apply reward
            _applyRedeemReward(reward);
            markCodeUsed(raw); // also mark locally so UI doesn't re-attempt
