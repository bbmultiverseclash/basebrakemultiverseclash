...
const RARITY_CFG = {
    ...
    Mythic:    { label:'Mythic',    color:'#f43f5e', glow:'rgba(244,63,94,0.9)',   border:'#e11d48', weight:0,  emoji:'💠' },
};
...
const ENHANCE_RECIPES = {
    'Godspeed Killua': {
        baseCard: 'Killua|hxh',
        materials: [
            { name: 'Hunter License', theme: 'hxh', count: 2 },
            { name: 'Chimera Ant', theme: 'hxh', count: 1 }
        ],
        gems: 20, coins: 2000, result: 'Godspeed Killua|hxh'
    }
};
...
function loadPlayerData() {
    try { const r = localStorage.getItem(SAVE_KEY); if(r) {
        ...
        if (d.rodTokens === undefined) d.rodTokens = 0;
        if (d.rodTokensSpent === undefined) d.rodTokensSpent = 0;
...
function getCardRarity(name, data) {
    // 1. ถ้ามีการระบุ rarity ไว้ในการ์ดโดยตรง ให้ใช้ค่านั้นเลย! (รองรับ Custom rarity)
    if (data && data.rarity) return data.rarity;

    // 2. เช็คว่าการ์ดนี้เป็นผลลัพธ์จากการ Enhance หรือไม่
    // ถ้าใช่ ให้หาความแรร์ของร่างต้น แล้วบวกเพิ่มไป 1 ขั้น (Tier Up)
    if (typeof ENHANCE_RECIPES !== 'undefined') {
        const recipeKey = Object.keys(ENHANCE_RECIPES).find(k => ENHANCE_RECIPES[k].result.split('|')[0] === name);
        if (recipeKey) {
            const baseCardStr = ENHANCE_RECIPES[recipeKey].baseCard;
            const [baseName, baseTheme] = baseCardStr.split('|');
            const baseData = (typeof CardSets !== 'undefined' && CardSets[baseTheme]) ? CardSets[baseTheme][baseName] : null;
            const baseRarity = getCardRarity(baseName, baseData);

            const baseIdx = RARITY_ORDER.indexOf(baseRarity);
            if (baseIdx !== -1 && baseIdx < RARITY_ORDER.length - 1) {
                return RARITY_ORDER[baseIdx + 1]; // เพิ่ม 1 ขั้น (เช่น Legendary -> Mythic)
            }
            return baseRarity; // ถ้าเป็น Mythic อยู่แล้วก็คงเป็น Mythic
        }
    }

    // 3. ระบบเก่า (สำหรับการ์ดที่ไม่ได้กำหนด rarity และไม่ได้ Enhance)
    ...
    <div style="background:linear-gradient(135deg,#0f0c29,#1a1640,#1e1b4b);border:3px solid ${borderColor};border-radius:24px;
      ...
      <div style="padding:16px 18px">
        ...
        <div style="display:flex;gap:8px">
          ${(() => {
            const recipeKey = Object.keys(ENHANCE_RECIPES).find(rk => ENHANCE_RECIPES[rk].baseCard === `${name}|${theme}`);
            return recipeKey ? `<button onclick="openEnhanceModal('${recipeKey}')" style="flex:1;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:white;border:1.5px solid #a78bfa;padding:10px;border-radius:10px;font-weight:800;cursor:pointer;font-size:0.85rem">✨ Enhance</button>` : '';
          })()}
...
function addToDeck(name, theme) {
    ...
    // --- ส่วนที่แก้: จำกัด Champion 1 ใบต่อเด็ค ---
    const isThisChamp = cardData?.type === 'Champion' || cardData?.isChampion || cardData?.type === 'Champion';
    if (isThisChamp) {
        const hasAnyChampion = deckBuilderState.cards.some(c => {
            const d = CardSets[c.theme][c.name];
            return d?.type === 'Champion' || d?.isChampion;
        });
        if (hasAnyChampion) {
            showToast('🚫 ในเด็คมี Champion ได้เพียง 1 ใบเท่านั้น!', '#f87171');
            return;
        }
    }
    // ------------------------------------------
...
const ALL_TITLES = [
    ...
    { id: 'Haunted Hunter',      how: 'ใช้จ่าย Rod Token รวม 60 อัน' },
...
function checkTitleUnlocks() {
    ...
    // HxH Rod Token based
    if ((playerData.rodTokensSpent || 0) >= 60) unlockTitle('Haunted Hunter');
...
const REDEEM_CODES = {
    ...
    'HXHISHERE':           { rodTokens: 10, label: '🎣 HxH Is Here', oneTime: true },
    '45GEMSNOTBUG':        { gems: 45, label: '💎 45 Gems Not Bug', serverClaim: true, maxClaims: 2 },
...
function _applyRedeemReward(reward) {
    ...
    if (typeof reward.rodTokens === 'number')    playerData.rodTokens    = (playerData.rodTokens    || 0) + reward.rodTokens;
...
function _redeemRewardParts(reward) {
    ...
    if (reward.rodTokens)    parts.push(`+${reward.rodTokens} 🎣`);
...
async function redeemCode() {
    ...
    // ── SERVER-WIDE ONE-TIME (or limited): first N players globally win ──
    if (reward.serverClaim) {
        ...
        const maxClaims = reward.maxClaims || 1;
        ...
        try {
            const { committed } = await claimRef.transaction(current => {
                const count = current ? (current.count || 1) : 0;
                if (count >= maxClaims) return; // abort — full
                return {
                    count: count + 1,
                    maxClaims,
                    lastClaimedBy: getUserKey() || 'guest',
                    lastClaimedAt: Date.now()
                };
            });
            if (!committed) {
                const limitMsg = maxClaims > 1
                    ? `❌ โค้ดนี้ถูกใช้ครบ ${maxClaims} คนแล้ว!`
                    : '❌ โค้ดนี้ถูกใช้ไปแล้วโดยผู้เล่นคนอื่น!';
                msg.style.color='#f87171'; msg.textContent=limitMsg; return;
            }
            _applyRedeemReward(reward);
            markCodeUsed(raw);
            const parts = _redeemRewardParts(reward);
            const isFirst = true; // committed = we got in
            ...
            try {
                ...
                const cur = snap.val();
                const maxClaims = reward.maxClaims || 1;
                if (cur && (cur.count || 1) >= maxClaims) {
                    msg.style.color='#f87171'; msg.textContent='❌ โค้ดนี้ถูกใช้ครบแล้ว!'; return;
                }
                await claimRef.set({
                    count: (cur?.count || 0) + 1,
                    maxClaims,
...
// ─── ENHANCE SYSTEM ──────────────────────────────────────────────────────────
function openEnhanceModal(recipeKey) {
    const recipe = ENHANCE_RECIPES[recipeKey];
    const targetData = CardSets['hxh'][recipeKey]; 
    const ownedBase = playerData.collection[recipe.baseCard] || 0;
    const canDo = (ownedBase >= 1 && (playerData.coins||0) >= recipe.coins && (playerData.gems||0) >= recipe.gems && recipe.materials.every(m => (playerData.collection[`${m.name}|${m.theme}`]||0) >= m.count));
    ...
    ov.innerHTML = `
        <div style="background:linear-gradient(135deg,#1e1b4b,#0f172a);border:3px solid #7c3aed;border-radius:24px;padding:24px;max-width:380px;width:90%;text-align:center;">
            <div style="font-size:1.5rem;font-weight:900;color:#a78bfa;margin-bottom:15px">✨ Card Enhancement ✨</div>
            ...
            <div style="background:rgba(0,0,0,0.3);padding:10px;border-radius:10px;margin-bottom:10px;text-align:left;font-size:0.8rem">
                ...
                <div style="color:${playerData.coins >= recipe.coins ? '#fbbf24':'#f87171'}">🪙 Coins: ${playerData.coins}/${recipe.coins}</div>
                <div style="color:${playerData.gems >= recipe.gems ? '#93c5fd':'#f87171'}">💎 Gems: ${playerData.gems}/${recipe.gems}</div>
            </div>
...
function confirmEnhance(recipeKey) {
    const recipe = ENHANCE_RECIPES[recipeKey];
    ...
    // 2. วนลูปหักวัตถุดิบทั้งหมดใน materials (License และ Chimera Ant)
    ...
    // 3. หักเงินและเพชร
    playerData.coins -= recipe.coins;
    playerData.gems -= recipe.gems;
...
// ============================================================
// Community Tier Rating, Deck Import/Export, Extended Lore
// ============================================================

document.addEventListener('DOMContentLoaded', () => {

    // ==========================================
    // 1. ระบบ CARD TIER RATING (โหวต SSS ถึง F)
    // ==========================================
    const TIER_META = {
        9: { label: 'SSS', color: '#f43f5e' },
        8: { label: 'SS',  color: '#f97316' },
        7: { label: 'S',   color: '#fbbf24' },
        6: { label: 'A',   color: '#4ade80' },
        5: { label: 'B',   color: '#38bdf8' },
        4: { label: 'C',   color: '#818cf8' },
        3: { label: 'D',   color: '#a78bfa' },
        2: { label: 'E',   color: '#9ca3af' },
        1: { label: 'F',   color: '#6b7280' }
    };

    if (typeof window.showCardDetailModal === 'function') {
        const _origShowCardDetailModal = window.showCardDetailModal;
        window.showCardDetailModal = function(name, theme) {
            _origShowCardDetailModal.apply(this, arguments);
            const modal = document.getElementById('col-card-modal');
            if (!modal) return;
            const actionArea = modal.querySelector('div[style*="display:flex;gap:8px"]');
            if (actionArea) {
                const tierBox = document.createElement('div');
                tierBox.id = 'tier-rating-box';
                tierBox.style.cssText = 'background:#1f2937; border-radius:12px; padding:12px; margin-bottom:14px; border:1px solid #374151;';
                tierBox.innerHTML = '<div style="color:#9ca3af;font-size:0.8rem;text-align:center;">กำลังโหลดข้อมูล Tier...</div>';
                actionArea.parentNode.insertBefore(tierBox, actionArea);
                fetchAndRenderTier(name);
            }
        };
    }

    window.fetchAndRenderTier = function(cardName) {
        if (!firebaseReady || typeof db === 'undefined' || !db) {
            const box = document.getElementById('tier-rating-box');
            if (box) box.innerHTML = '<div style="color:#f87171;font-size:0.8rem;text-align:center;">ออฟไลน์โหมด (ไม่สามารถดู/โหวต Tier ได้)</div>';
            return;
        }

        // ── [FIX] timeout fallback: ถ้า 6 วินาทียังไม่ตอบ → แสดง offline ──
        const _tierFallback = setTimeout(() => {
            const box = document.getElementById('tier-rating-box');
            if (box && box.innerHTML.includes('กำลังโหลด')) {
                box.innerHTML = '<div style="color:#f87171;font-size:0.8rem;text-align:center;">ออฟไลน์โหมด (timeout)</div>';
            }
        }, 6000);

        db.ref('globalCardRatings/' + cardName).once('value').then(snap => {
            clearTimeout(_tierFallback);
            const votes = snap.val() || {};
            const uids = Object.keys(votes);
            let total = 0;
            let myVote = (typeof currentUser !== 'undefined' && currentUser) ? votes[currentUser.uid] : null;
            uids.forEach(uid => total += votes[uid]);
            let avgScore = uids.length > 0 ? Math.round(total / uids.length) : 0;
            let tierLabel = avgScore > 0 ? TIER_META[avgScore].label : '?';
            let tierColor = avgScore > 0 ? TIER_META[avgScore].color : '#6b7280';
            const box = document.getElementById('tier-rating-box');
            if (!box) return;
            let html = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <div style="font-size:0.8rem; color:#9ca3af; font-weight:bold;">🏆 Community Tier</div>
                    <div style="display:flex; align-items:center; gap:8px;">
                        <span style="font-size:0.7rem; color:#6b7280;">(${uids.length} โหวต)</span>
                        <span style="background:${tierColor}; color:${avgScore > 0 ? '#000' : '#fff'}; padding:2px 10px; border-radius:8px; font-weight:900; font-size:1rem; box-shadow:0 0 10px ${tierColor}66;">${tierLabel}</span>
                    </div>
                </div>`;
            if (typeof currentUser !== 'undefined' && currentUser && !currentUser.isAnonymous) {
                html += `<div style="font-size:0.7rem; color:#9ca3af; margin-bottom:5px;">โหวตให้คะแนนการ์ดใบนี้:</div>
                         <div style="display:flex; gap:4px; overflow-x:auto; padding-bottom:4px; scrollbar-width:none;">`;
                [9, 8, 7, 6, 5, 4, 3, 2, 1].forEach(score => {
                    const label = TIER_META[score].label;
                    const isMy = myVote == score;
                    const bg = isMy ? TIER_META[score].color : '#374151';
                    const col = isMy ? '#000' : 'white';
                    const border = isMy ? 'none' : `1px solid ${TIER_META[score].color}44`;
                    html += `<button onclick="voteCardTier('${cardName}', ${score})" style="flex-shrink:0; background:${bg}; color:${col}; border:${border}; padding:4px 8px; border-radius:6px; font-weight:bold; font-size:0.75rem; cursor:pointer;">${label}</button>`;
                });
                html += `</div>`;
            } else {
                html += `<div style="font-size:0.7rem; color:#6b7280; text-align:center;">🔒 ล็อกอินด้วย Google เพื่อร่วมโหวต Tier</div>`;
            }
            box.innerHTML = html;
        }).catch(() => {
            // ── [FIX] Firebase error → แสดง offline แทน loading ค้าง ──
            clearTimeout(_tierFallback);
            const box = document.getElementById('tier-rating-box');
            if (box) box.innerHTML = '<div style="color:#f87171;font-size:0.8rem;text-align:center;">ออฟไลน์โหมด (ไม่สามารถดู/โหวต Tier ได้)</div>';
        });
    ...
    window.voteCardTier = function(cardName, score) {
        if (!currentUser || currentUser.isAnonymous) return;
        db.ref('globalCardRatings/' + cardName + '/' + currentUser.uid).set(score).then(() => {
            fetchAndRenderTier(cardName);
            if (typeof showToast === 'function') showToast('✅ โหวต Tier สำเร็จ!', '#4ade80');
        });
    };

    // ==========================================
    // 2. ระบบ IMPORT / EXPORT DECK
    // ==========================================
    if (typeof window.renderDeckBuilderPanel === 'function') {
        const _origRenderDeckBuilderPanel = window.renderDeckBuilderPanel;
        window.renderDeckBuilderPanel = function() {
            _origRenderDeckBuilderPanel.apply(this, arguments);
            const pnl = document.getElementById('hub-panel-deckbuilder');
            if (!pnl) return;
            const allDivs = Array.from(pnl.querySelectorAll('div'));
            const deckHeader = allDivs.find(d => d.textContent.trim().includes('เด็คของฉัน') && d.children.length === 0); // [FIX] ใช้ includes() แทน === เพราะ textContent จริงมี emoji 🃏 นำหน้า
            if (deckHeader) {
                const parent = deckHeader.parentElement;
                const btnGroup = document.createElement('div');
                btnGroup.style.cssText = 'display:flex; gap:8px;';
                btnGroup.innerHTML = `
                    <button onclick="importDeckList()" style="background:#1d4ed8;color:white;border:none;padding:5px 10px;border-radius:6px;font-size:0.7rem;cursor:pointer;font-weight:bold;box-shadow:0 0 10px rgba(29,78,216,0.5);">📥 Import</button>
                    <button onclick="exportDeckList()" style="background:#059669;color:white;border:none;padding:5px 10px;border-radius:6px;font-size:0.7rem;cursor:pointer;font-weight:bold;box-shadow:0 0 10px rgba(5,150,105,0.5);">📤 Export</button>`;
                const newHeaderWrap = document.createElement('div');
                newHeaderWrap.style.cssText = 'display:flex; justify-content:space-between; align-items:center; width:100%;';
                newHeaderWrap.innerHTML = `<div style="font-weight:800;color:white">🃏 เด็คของฉัน</div>`;
                newHeaderWrap.appendChild(btnGroup);
                parent.replaceChild(newHeaderWrap, deckHeader);
            }
        };
    }

    window.exportDeckList = function() {
        if (typeof deckBuilderState === 'undefined' || deckBuilderState.cards.length === 0) {
            if (typeof showToast === 'function') showToast('❌ เด็คว่างเปล่า ไม่สามารถ Export ได้', '#f87171');
            return;
        }
        const dataStr = JSON.stringify(deckBuilderState.cards.map(c => ({n: c.name, t: c.theme})));
        const code = btoa(encodeURIComponent(dataStr));
        navigator.clipboard.writeText(code).then(() => {
            if (typeof showToast === 'function') showToast('📋 คัดลอก Deck Code แล้ว! นำไปส่งให้เพื่อนได้เลย', '#4ade80');
        }).catch(() => {
            prompt('ไม่สามารถคัดลอกอัตโนมัติได้ ให้คัดลอกโค้ดด้านล่างนี้แทน:', code);
        });
    };

    window.importDeckList = function() {
        const code = prompt('📥 วาง Deck Code ที่เพื่อนส่งมาที่นี่:');
        if (!code) return;
        try {
            ...
            let missing = [];
            let addedCount = 0;
            let myCol = { ...playerData.collection };
            deckBuilderState.cards.forEach(c => {
                const k = `${c.name}|${c.theme}`;
                if (myCol[k]) myCol[k]--;
            });
            parsed.forEach(c => {
                const name = c.n;
                ...
                if (!cardData) { missing.push(name); return; }
                const isChampion = cardData.isChampion || cardData.type === 'Champion';
                const maxAllowed = isChampion ? 1 : 3;
                const countInDeck = deckBuilderState.cards.filter(x => x.name === name && x.theme === theme).length;
                if ((myCol[k] || 0) > 0 && deckBuilderState.cards.length < 65 && countInDeck < maxAllowed) {
                    if (isChampion) {
                        const hasAnyChampion = deckBuilderState.cards.some(x => {
                            const d = CardSets[x.theme]?.[x.name];
                            return d?.isChampion || d?.type === 'Champion';
                        });
                        if (hasAnyChampion) { missing.push(name + ' (จำกัด Champion 1 ใบ)'); return; }
                    }
                    deckBuilderState.cards.push({name, theme});
                    myCol[k]--;
                    addedCount++;
            ...
            if (missing.length > 0) {
                ...
                alert(`✅ เพิ่มการ์ดได้ ${addedCount} ใบ\n\n❌ ขาดการ์ด (หรือเกินลิมิต):\n- ${uniqueMissing.join('\n- ')}`);
            } else {
                if (typeof showToast === 'function') showToast(`✅ Import สำเร็จ! เพิ่ม ${addedCount} ใบ`, '#4ade80');
            }
        } catch(e) {
            console.error(e);
            if (typeof showToast === 'function') showToast('❌ Deck Code ไม่ถูกต้อง!', '#f87171');
        }
    };

    // ==========================================
    // 3. EXTENDED LORE (ประวัติการ์ดเพิ่มเติม)
    // ==========================================
    const EXTENDED_LORE = {
        // --- Mythology ---
        'Zeus':        { title: '★ กษัตริย์แห่งทวยเทพ',          lore: 'ผู้ปกครองโอลิมปัส สายฟ้าของเขาเปรียบดั่งการลงทัณฑ์จากสวรรค์ เมื่อพิโรธ ท้องฟ้าจะฉีกขาดและกวาดล้างศัตรูทั้งหมด' },
        'Poseidon':    { title: '★ จ้าวแห่งมหาสมุทร',             lore: 'ผู้ปกครองผืนน้ำและผู้ทำให้เกิดแผ่นดินไหว ตรีศูลของเขาสามารถทำให้มหาสมุทรแยกออกและจมกองทัพลงสู่ก้นบึ้ง' },
        'Hades':       { title: '★ จ้าวแห่งยมโลก',                lore: 'ผู้ปกครองดินแดนคนตาย ผู้เฝ้าวิญญาณไม่ให้หลบหนี การปรากฏตัวของเขานำมาซึ่งความหนาวเหน็บแห่งขุมนรก' },
        'Odin':        { title: '★ บิดาแห่งทวยเทพนอร์ส',          lore: 'ผู้ยอมแลกจักษุหนึ่งข้างเพื่อดื่มน้ำพุแห่งปัญญา เขารู้ล่วงหน้าถึงวันแร็กนาร็อกและเตรียมพร้อมสู้อย่างไม่ลดละ' },
        'Thor':        { title: '✦ เทพเจ้าสายฟ้า',                lore: 'นักรบที่แข็งแกร่งที่สุดในแอสการ์ด ผู้ถือค้อนมโยลเนียร์เพื่อปกป้องมิดการ์ดจากเหล่ายักษ์และอสูรกาย' },
        'Loki':        { title: '✦ เทพแห่งการหลอกลวง',            lore: 'ผู้สร้างความวุ่นวายด้วยเวทมนตร์จำแลงกาย ไม่มีใครรู้ว่าเขาอยู่ฝ่ายใดจนกว่ามีดจะแทงเข้าที่หลัง' },
        'King Arthur': { title: '✦ กษัตริย์แห่งบริเตน',           lore: 'กษัตริย์ในตำนานผู้ดึงดาบออกจากหิน นำพาเหล่าอัศวินโต๊ะกลมเพื่อรวบรวมแผ่นดินให้เป็นหนึ่งเดียว' },
        'Shiva':       { title: '★ เทพผู้ทำลายล้าง',              lore: 'เมื่อพระศิวะเปิดตาที่สาม จักรวาลทั้งมวลจะถูกแผดเผาเป็นเถ้าถ่าน เพื่อเปิดทางให้กับการสร้างสรรค์ใหม่' },
        'Rama':        { title: '★ อวตารแห่งพระนารายณ์',           lore: 'วีรบุรุษผู้ถือธนูศรพรหมาสตร์ ศรของเขาไม่เคยพลาดเป้าและสามารถทำลายความมืดมิดทั้งปวงได้' },
        'Tossakan':    { title: '★ พญายักษ์สิบหน้า',              lore: 'ราชาแห่งลงกา ผู้มีสิบหน้ายี่สิบมือและหัวใจที่ถูกถอดออกไปซ่อน ทำให้เขาเป็นอมตะตราบที่หัวใจไม่ถูกทำลาย' },
        'Gilgamesh':   { title: '★ ราชาแห่งวีรชน',                lore: 'กษัตริย์แห่งอุรุก ผู้ครอบครองคลังสมบัติแห่งบาบิโลนที่มีอาวุธต้นแบบของทุกตำนานในโลก' },
        'Hercules':    { title: '★ วีรบุรุษสิบสองภารกิจ',          lore: 'บุตรแห่งซุสผู้มีพละกำลังเหนือมนุษย์ การผ่านภารกิจทั้ง 12 ทำให้เขากลายเป็นสัญลักษณ์แห่งความไม่ยอมแพ้' },
        // --- Animal Kingdom ---
        'Tiger':                 { title: '✦ นักล่าลายพรางแห่งป่า',  lore: 'นักล่าเดี่ยวที่ล้มเหยื่อหนักกว่าตัวเองหลายเท่า ลายทางช่วยกลมกลืนในป่าดิบ ก้าวย่างเงียบราวไม่มีตัวตน' },
        'Lion King of Forest':   { title: '★ ราชาแห่งป่า',            lore: 'สิงโตถูกขนานนามว่า "ราชาแห่งป่า" เสียงคำรามดังได้ยินระยะ 8 กม. และนำฝูงด้วยความสง่างาม' },
        'Blue Whale':            { title: '★ เจ้าแห่งมหาสมุทร',       lore: 'วาฬสีน้ำเงินเป็นสัตว์ที่ใหญ่ที่สุดที่เคยมีชีวิตอยู่บนโลก หนักถึง 200 ตัน หัวใจใหญ่เท่ารถยนต์' },
        'Silverback Gorilla':    { title: '✦ เจ้าแห่งป่าดิบ',          lore: 'ตัวผู้โตเต็มวัยที่มีพละกำลังมหาศาล เสียงคำรามและการทุบอกของมันทำให้ศัตรูต้องถอยหนี' },
        'Komodo Dragon':         { title: '✦ มังกรพิษ',               lore: 'สัตว์เลื้อยคลานที่ใหญ่ที่สุด น้ำลายมีพิษและแบคทีเรียที่ทำให้เหยื่อค่อยๆ อ่อนแรงลงจนตาย' },
        'Grizzly Bear':          { title: '✦ ราชาแห่งอเมริกาเหนือ',   lore: 'หมีร่างยักษ์ที่มีกรงเล็บคมกริบ เมื่อโกรธ แม้แต่สัตว์ใหญ่ก็ไม่กล้าเผชิญหน้า' },
        'Anaconda':              { title: '✦ อสูรกายใต้น้ำ',           lore: 'งูที่หนักที่สุดในโลก ซุ่มซ่อนอยู่ใต้น้ำเพื่อพันรัดเหยื่อด้วยแรงบีบหลายตันก่อนกลืนลงไปทั้งตัว' },
        // --- Toy Trooper ---
        'King':         { title: '✦ หมากรุกราชา',        lore: 'ผู้นำสูงสุดบนกระดาน แม้จะเดินได้ทีละก้าว แต่การมีอยู่ของเขาคือสิ่งเดียวที่ตัดสินผลแพ้ชนะ' },
        'Queen':        { title: '✦ หมากรุกราชินี',       lore: 'หมากที่ทรงพลังที่สุดบนกระดาน สามารถเคลื่อนที่กวาดล้างศัตรูได้ในทุกทิศทางอย่างรวดเร็ว' },
        'Teddy Bear':   { title: 'ตุ๊กตาหมีผู้อบอุ่น',    lore: 'ของเล่นวัยเด็กที่คอยปกป้องเราจากฝันร้ายในยามค่ำคืน แต่หากมันโกรธ มันจะกลายเป็นความบ้าคลั่ง' },
        'Toy-Rex':      { title: '✦ ของเล่นไดโนเสาร์',   lore: 'พลาสติกที่แข็งแกร่งที่สุดในกล่องของเล่น เมื่อถูกปลุกขึ้นมา มันจะกระทืบและพ่นไฟแห่งจินตนาการ' },
        'Gundam Model': { title: '✦ หุ่นประกอบโมเดล',    lore: 'หุ่นพลาสติกที่ถูกประกอบขึ้นอย่างตั้งใจ พร้อมด้วยอาวุธและชุดเกราะเสริมที่ถูกดัดแปลงมาอย่างดี' },
        // --- Humanity ---
        'Socrates':       { title: '✦ บิดาแห่งปรัชญา',              lore: 'ผู้เชื่อว่า "ข้าพเจ้ารู้เพียงสิ่งเดียว คือข้าพเจ้าไม่รู้อะไรเลย" คำถามของเขาสั่นคลอนความเชื่อของผู้คน' },
        'Newton':         { title: '✦ บิดาแห่งแรงโน้มถ่วง',          lore: 'ผู้ค้นพบกฎแห่งแรงโน้มถ่วงและกฎการเคลื่อนที่ เขาสามารถคำนวณและลดทอนความพยายามของศัตรูได้อย่างแม่นยำ' },
        'Charles Darwin': { title: '✦ ผู้ค้นพบวิวัฒนาการ',            lore: 'ผู้เสนอทฤษฎีการคัดเลือกโดยธรรมชาติ สิ่งมีชีวิตที่ปรับตัวได้ดีที่สุดเท่านั้นจึงจะอยู่รอด' },
        'Luis Pasteur':   { title: '✦ ผู้พิทักษ์แห่งจุลชีววิทยา',    lore: 'ผู้ค้นพบวัคซีนและการฆ่าเชื้อ การค้นพบของเขาช่วยปกป้องสิ่งมีชีวิตจากการเน่าเปื่อย' },
        'Messi':          { title: '★ อัจฉริยะลูกหนัง',               lore: 'นักเตะผู้ก้าวข้ามขีดจำกัดของมนุษย์ การเลี้ยงบอลของเขาไหลลื่นราวกับเวทมนตร์ เมื่อทำแฮตทริก เขาจะไร้เทียมทาน' },
        'Schrödinger':    { title: '★ แมวในกล่องควอนตัม',             lore: 'การทดลองทางความคิดที่แมวอาจตายหรือมีชีวิตอยู่พร้อมกัน จนกว่าคุณจะเปิดกล่องดูความจริง' },
        'Mozart':         { title: '✦ อัจฉริยะนักประพันธ์',           lore: 'ผู้เปลี่ยนโน้ตดนตรีให้กลายเป็นเวทมนตร์ ทุกจังหวะและท่วงทำนองจะบรรเลงเพื่อบัฟพลังให้พันธมิตร' },
        'Achimedes':      { title: '✦ นักปราชญ์แห่งซีราคิวส์',        lore: '"หาจุดยืนให้ข้าสิ แล้วข้าจะงัดโลกทั้งใบให้ดู" กลไกของเขาสามารถป่วนสนามรบได้อย่างคาดไม่ถึง' },
        // --- JJK ---
        'Gojo Satoru (Shibuya Station)': { title: '♛ แชมเปี้ยนแห่งมุเก็น',      lore: '"ท่ามกลางสวรรค์และโลกหล้า ข้าคือผู้เป็นเลิศเพียงหนึ่งเดียว" เมื่อถอดผ้าปิดตาออก ทุกสิ่งจะถูกกลืนกินในอาณาเขตไร้มาตร' },
        'Ryomen Sukuna (Full Power)':    { title: '♛ ราชาคำสาปที่แท้จริง',       lore: 'คำสาปที่ทรงพลังที่สุดในยุคเฮอัน การฟันของเขาสามารถตัดได้แม้กระทั่งมิติและอวกาศ ไม่มีผู้ใดต้านทาน Malevolent Shrine ได้' },
        'Yuji Itadori':                  { title: '✦ ภาชนะของราชาคำสาป',         lore: 'เด็กหนุ่มผู้กลืนนิ้วของสุคุนะและรอดชีวิตมาได้ ร่างกายเหนือมนุษย์และหมัดประกายทมิฬ (Black Flash) คืออาวุธของเขา' },
        'Toji Fushiguro':                { title: '✦ นักฆ่าผู้ถูกสวรรค์ทอดทิ้ง', lore: 'ผู้ไร้พลังเวทมนตร์โดยกำเนิด แต่แลกมาด้วยข้อผูกมัดสวรรค์ที่ให้พละกำลังและสัมผัสทางกายเหนือมนุษย์' },
        'Megumi Fushiguro':              { title: '✦ ผู้ใช้วิชาเงาทั้งสิบ',       lore: 'ผู้สืบทอดสายเลือดตระกูลเซนอิง สามารถอัญเชิญชิกิงามิจากเงา และเป็นคนเดียวที่สุคุนะให้ความสนใจ' },
        'Mahoraga':                      { title: '★ มโหราค ดาบแห่งการปรับตัว',   lore: 'ชิกิงามิที่แข็งแกร่งที่สุดในวิชาเงาทั้งสิบ มีความสามารถในการ "ปรับตัว" ต่อทุกปรากฏการณ์หรือการโจมตีใดๆ ที่โดนไปแล้ว' },
        'Kenjaku':                       { title: '★ นักสาปแช่งอายุพันปี',         lore: 'จอมบงการผู้อาศัยการเปลี่ยนสมองเพื่อย้ายร่างไปเรื่อยๆ ผู้มีความรู้ด้านวิชาคุณไสยลึกซึ้งที่สุดและเป็นผู้วางแผนทั้งหมด' },
        // --- Re:Zero ---
        'Subaru v2':                     { title: '✦ ผู้แบกรับความตาย',             lore: 'Natsuki Subaru หนุ่มธรรมดาผู้ถือครองพลัง "Return by Death" แม้จะต้องตายซ้ำแล้วซ้ำเล่า เขาก็จะลุกขึ้นมาใหม่เพื่อปกป้องเอมิเลีย' },
        'Rem':                           { title: '✦ เมดสาวเผ่ายักษ์',              lore: 'น้องสาวฝาแฝดของแรม ผู้ทุ่มเทชีวิตเพื่อคนที่เธอรัก หากมีคนทำร้ายสุบารุ เธอจะควงลูกตุ้มเหล็กทำลายล้างทุกสิ่ง' },
        'Ram (The Awakened Oni Goddess)':{ title: '★ เทพธิดายักษ์ตื่นรู้',          lore: 'อดีตเด็กอัจฉริยะเผ่ายักษ์ แม้จะสูญเสียเขาไปแล้ว แต่เมื่อผสานพลังกับน้องสาว เธอสามารถร่ายเวทสายลมที่ฉีกกระชากทุกสิ่งได้' },
        'Reinhard (Full Power)':         { title: '★ อัศวินดาบศักดิ์สิทธิ์ที่แท้จริง', lore: 'ชายผู้เป็นที่รักของโลก ได้รับพรจากพระเจ้าในทุกๆ ด้าน เขาคือตัวตนที่บั๊กที่สุดและไม่มีวันถูกเอาชนะได้อย่างสมบูรณ์' },
        'Puck, Beast of the End':        { title: '★ สัตว์ร้ายแห่งจุดจบ',           lore: 'เมื่อเอมิเลียตาย พัคจะปลดปล่อยร่างที่แท้จริง พายุหิมะแห่งความหนาวเหน็บจะกลืนกินโลกทั้งใบเพื่อเป็นพิธีศพให้เธอ' },
        // --- Holy War (10 Commandments) ---
        'Demon King':  { title: '♛ ราชาแห่งโลกปีศาจ',      lore: 'บิดาของเมลิโอดัสและเซลดริส ผู้มีพลัง The Ruler ที่แปรเปลี่ยนทุกการโจมตีหรือจุดอ่อนให้กลายเป็นการฟื้นฟู' },
        'Zeldris':     { title: '★ ตัวแทนราชาปีศาจ',        lore: 'ผู้นำของบัญญัติ 10 ประการ ผู้ครอบครองพลัง Ominous Nebula ที่ดึงดูดและทำลายทุกสิ่งรอบตัวในเสี้ยววินาที' },
        'Estarossa':   { title: '★ บัญญัติแห่งความรัก',     lore: 'ผู้มีรอยยิ้มอันอ่อนโยน แต่ความรักของเขาทำให้ใครก็ตามที่คิดร้ายสูญเสียพลัง และวิชา Full Counter ของเขาสะท้อนทุกการโจมตีกายภาพ' },
        'Gloxinia':    { title: '★ ปฐมกษัตริย์ภูต',         lore: 'อดีตผู้พิทักษ์ป่าศักดิ์สิทธิ์ ผู้ร่ายรำพร้อมหอก Basquias ที่มีรูปแบบการโจมตีและการรักษาถึง 10 รูปแบบ' },
        'Drole':       { title: '★ ราชาเผ่ายักษ์',           lore: 'ผู้สถาปนาเผ่ายักษ์และคิดค้นการเต้นรำแห่งผืนดิน ร่างกายของเขาสูงใหญ่เหมือนภูเขาและแข็งแกร่งดั่งแร่เพชร' },
        'Galand':      { title: '★ บัญญัติแห่งความจริง',    lore: 'ใครก็ตามที่พูดโกหกต่อหน้าเขา ร่างกายจะกลายเป็นหินถาวร เขาเป็นนักรบที่บ้าคลั่งและหลงใหลในการต่อสู้' },
        'Melascula':   { title: '★ บัญญัติแห่งความเชื่อ',   lore: 'ผู้ควบคุมวิญญาณและความตาย เธอสามารถดึงวิญญาณออกจากร่างและเปิดประตูมิติสู่ดินแดนแห่งคนตายได้' },
        'Grayroad':    { title: '★ บัญญัติแห่งความสงบ',     lore: 'ปีศาจกลายพันธุ์ที่ไม่มีเพศ ผู้ใดที่คร่าชีวิตต่อหน้าเธอ จะถูกสูบเวลาชีวิตจนตายตามไป เป็นคำสาปที่สมบูรณ์แบบ' },
        // --- Frieren ---
        'Frieren':          { title: '★ เมจแห่งการเดินทางชั่วนิรันดร์', lore: 'เอลฟ์ผู้มีชีวิตอยู่มานานกว่าพันปี อดีตสมาชิกปาร์ตี้ผู้กล้าฮิมเมล รักการสะสมเวทมนตร์แปลกๆ ปริมาณมานาของเธอมหาศาลจนหยั่งไม่ถึง' },
        'Fern the Sniper':  { title: '✦ จอมเวทปืนกล',                    lore: 'ลูกศิษย์ของฟรีเรน ผู้เชี่ยวชาญการยิง Zoltraak รัวและเร็วอย่างน่าเหลือเชื่อ การควบคุมมานาของเธอละเอียดอ่อนและสมบูรณ์แบบ' }
    };

    // รวม Lore เข้ากับระบบหลัก
    if (typeof CARD_LORE !== 'undefined') {
        Object.assign(CARD_LORE, EXTENDED_LORE);
    }
});
