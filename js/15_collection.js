...
function loadPlayerData() {
    try { const r = localStorage.getItem(SAVE_KEY); if(r) {
        ...
        if (d.rzPacksBought === undefined) d.rzPacksBought = 0;
...
function getSetCardPool(setName) {
    ...
    return Object.entries(cards)
        // กรองเอาเฉพาะตัวที่ไม่ใช่ Champion และ ไม่ใช่ shopOnly
        .filter(([, data]) => !data.isChampion && !data.shopOnly)
        .map(([name, data]) => ({
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof endGame !== 'undefined') {
        ...
        endGame = function(winner) {
            ...
            // ── ล้าง reward เก่าทุกครั้งก่อนแสดงใหม่ ──
            document.getElementById('_ranked-reward-display')?.remove();
            document.getElementById('_quickplay-xp-display')?.remove();

            if (_isRankedGame) {
                // Ranked: แสดง XP + RP + Coins ใน rematch modal (Stat UI)
                const reward = isPlayerWin ? awardWin() : awardLoss();
                _showRewardInModal(isPlayerWin, reward);
                _isRankedGame = false;
            } else if (gameMode === 'ai') {
                // Quick Play vs AI — แสดง XP ใน rematch modal เบาๆ
                const xpGain = isPlayerWin ? 50 : 25;
                addXp(xpGain);
                _showQuickPlayXpInModal(xpGain, isPlayerWin);
            } else if (gameMode === 'online') {
                // Quick Play Online — แสดง XP ใน rematch modal เบาๆ
...
// ─── REWARD IN MODAL ─────────────────────────────────────────────
// แสดง XP / RP / Coins ตรงใน rematch-modal แทน floating overlay
function _showRewardInModal(isWin, reward) {
    setTimeout(() => {
        // ลบ reward เดิม (ถ้ามี)
        document.getElementById('_ranked-reward-display')?.remove();

        const modal = document.getElementById('rematch-modal');
        const anchor = document.getElementById('rematch-winner') || document.getElementById('rematch-title');
        if (!modal || !anchor) {
            // Fallback: ถ้าไม่เจอ modal ให้ใช้ toast
            _showRewardToastFallback(isWin, reward);
            return;
        }

        const div = document.createElement('div');
        div.id = '_ranked-reward-display';
        div.style.cssText = [
            'margin:10px auto 0',
            'max-width:280px',
            'background:' + (isWin ? 'linear-gradient(135deg,rgba(4,120,87,0.4),rgba(6,95,70,0.3))' : 'linear-gradient(135deg,rgba(127,29,29,0.4),rgba(90,20,20,0.3))'),
            'border:1.5px solid ' + (isWin ? '#34d399' : '#f87171'),
            'border-radius:14px',
            'padding:12px 20px',
            'text-align:center',
        ...
        const rank = typeof getRankInfo === 'function' ? getRankInfo((typeof playerData !== 'undefined' ? playerData.rp : 0)) : null;
        const rankBadge = rank ? `<div style="font-size:0.68rem;color:#9ca3af;margin-bottom:6px;">${rank.emoji} ${rank.name} Ranked Reward</div>` : '';
        ...
        if (isWin) {
            div.innerHTML = `
                ...
                <div style="display:flex;justify-content:center;gap:18px;flex-wrap:wrap;">
                    <div>
                        <div style="font-size:1.15rem;font-weight:900;color:#fbbf24">+${reward.coins} 🪙</div>
                        <div style="font-size:0.62rem;color:#9ca3af;">Coins</div>
                    </div>
                    <div>
                        <div style="font-size:1.15rem;font-weight:900;color:#818cf8">+${reward.rp} RP</div>
                        <div style="font-size:0.62rem;color:#9ca3af;">Rank Points</div>
                    </div>
                    <div>
                        <div style="font-size:1.15rem;font-weight:900;color:#6ee7b7">+${reward.xp||0} ⭐</div>
                        <div style="font-size:0.62rem;color:#9ca3af;">XP</div>
                    </div>
                </div>
                ${reward.anutinBonus ? `<div style="margin-top:6px;font-size:0.72rem;color:#34d399;font-weight:800">🌿 Anutin Bonus +${reward.anutinBonus} 🪙</div>` : ''}`;
        } else {
            div.innerHTML = `
                ${rankBadge}
                <div style="display:flex;justify-content:center;gap:18px;flex-wrap:wrap;">
                    <div>
                        <div style="font-size:1.15rem;font-weight:900;color:#f87171">-${reward.rpLoss} RP</div>
                        <div style="font-size:0.62rem;color:#9ca3af;">Rank Points</div>
                    </div>
                    <div>
                        <div style="font-size:1.15rem;font-weight:900;color:#6ee7b7">+${reward.xp||0} ⭐</div>
        ...
        // แทรกหลัง anchor (rematch-winner)
        anchor.insertAdjacentElement('afterend', div);
    }, 120);
}

// Fallback สำหรับกรณีที่ modal ไม่โหลด
function _showRewardToastFallback(isWin, reward) {
    ...
    toast.style.cssText = `position:fixed;top:30%;left:50%;transform:translateX(-50%);
        ...
        border:3px solid ${isWin?'#4ade80':'#f87171'};border-radius:20px;padding:20px 36px;
        z-index:9999;text-align:center;box-shadow:0 0 60px ${isWin?'rgba(74,222,128,0.5)':'rgba(248,113,113,0.5)'};
        animation:toastIn 0.4s ease;min-width:220px;`;
    toast.innerHTML = isWin
        ? `<div style="color:#fbbf24;font-weight:800">+${reward.coins} 🪙 &nbsp; +${reward.rp} RP &nbsp; +${reward.xp||0} ⭐</div>`
        : `<div style="color:#f87171;font-weight:800">-${reward.rpLoss} RP &nbsp; +${reward.xp||0} ⭐</div>`;
    document.body.appendChild(toast);
    setTimeout(() => { toast.style.animation='toastOut 0.4s ease forwards'; setTimeout(()=>toast.remove(),400); }, 3200);
}

// ─── QUICK PLAY XP IN MODAL ──────────────────────────────────────
// แสดง XP สำหรับ Quick Play (ไม่ใช่ Ranked) ใน rematch modal เบาๆ
function _showQuickPlayXpInModal(xpGain, isWin) {
    setTimeout(() => {
        document.getElementById('_quickplay-xp-display')?.remove();
        const anchor = document.getElementById('rematch-winner') || document.getElementById('rematch-title');
        if (!anchor) { showToast(`+${xpGain} ⭐ XP`, isWin ? '#4ade80' : '#9ca3af'); return; }
        const div = document.createElement('div');
        div.id = '_quickplay-xp-display';
        div.style.cssText = 'margin:8px auto 0;max-width:260px;background:rgba(0,0,0,0.25);border:1px solid #374151;border-radius:10px;padding:7px 16px;text-align:center;animation:fadeIn 0.3s ease;';
        div.innerHTML = `<div style="font-size:0.9rem;font-weight:800;color:#6ee7b7">+${xpGain} ⭐ XP</div>
            <div style="font-size:0.62rem;color:#6b7280;margin-top:1px">Quick Play</div>`;
        anchor.insertAdjacentElement('afterend', div);
    }, 120);
}
