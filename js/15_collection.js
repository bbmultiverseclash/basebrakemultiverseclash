...
function saveData() {
    ...
    if (typeof db !== 'undefined' && db &&
        ...
        try {
            // ── [FIX] strip invalid fields ก่อน save ขึ้น Firebase ──
            // Firebase ไม่รองรับ undefined, function, class instance
            const clean = JSON.parse(JSON.stringify(playerData));
            db.ref('playerSave/' + currentUser.uid).set(clean).catch(e => {
                console.warn('[CloudSave] auto-save failed:', e.code || e.message);
            });
        } catch(e) {}
    }
}
...
async function manualSaveToCloud() {
    if (typeof db === 'undefined' || !db || !currentUser || currentUser.isAnonymous) {
        if (typeof showToast === 'function') showToast('⚠️ ต้อง Login ก่อน', '#f87171');
        return;
    }
    try {
        const clean = JSON.parse(JSON.stringify(playerData));
        await db.ref('playerSave/' + currentUser.uid).set(clean);
        localStorage.setItem(SAVE_KEY, JSON.stringify(playerData));
        if (typeof showToast === 'function') showToast('☁️ Save สำเร็จ!', '#4ade80');
    } catch(e) {
        if (typeof showToast === 'function') showToast('❌ Save ไม่สำเร็จ: ' + (e.code || e.message), '#f87171');
    }
}
...
function renderHomePanel() {
    ...
    const cloudBanner = isLoggedIn
        ? `<div style="background:linear-gradient(135deg,#064e3b,#065f46);border:1px solid #10b981;border-radius:14px;padding:12px 16px;display:flex;align-items:center;gap:10px">
            ...
            <button onclick="manualSaveToCloud()" style="background:#065f46;color:#34d399;border:1px solid #10b981;border-radius:8px;padding:6px 10px;font-size:0.72rem;cursor:pointer;font-weight:700">💾 Save</button>
          </div>`
