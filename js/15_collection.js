...
async function loadPlayerDataFromFirebase(uid) {
    ...
    try {
        ...
        // ── Login Reward: 2000 coins + 5 gems (ครั้งเดียวต่อบัญชี) ──
        if (!playerData.loginRewardClaimed) {
            ...
            // บันทึกตรงๆ ไม่ผ่าน saveData() เพื่อหลีกเลี่ยง loop
            try { localStorage.setItem(SAVE_KEY, JSON.stringify(playerData)); } catch(e) {}
            try { db.ref('playerSave/' + uid).set(playerData); } catch(e) {}
            if (typeof updateHubUI === 'function') updateHubUI();
            // แสดง popup
            setTimeout(() => {
                const overlay = document.createElement('div');
                overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:10200;display:flex;align-items:center;justify-content:center';
                overlay.innerHTML = `
                <div style="background:linear-gradient(135deg,#1c1640,#2d2060);border:3px solid #fbbf24;border-radius:24px;padding:32px 28px;max-width:360px;width:92%;text-align:center;box-shadow:0 0 80px rgba(251,191,36,0.5)">
                  ...
                  <button onclick="this.closest('div[style*=fixed]').remove();if(typeof updateHubUI==='function')updateHubUI();"
                    style="background:linear-gradient(135deg,#d97706,#b45309);color:white;border:none;padding:12px 36px;border-radius:14px;font-weight:900;font-size:1rem;cursor:pointer">
                    🎉 รับเลย!
                  </button>
                </div>`;
