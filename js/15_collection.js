    ...
    <div style="padding:16px;max-width:900px;margin:0 auto">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-start">
        ${filtered.map(c => {
            ...
            return `
            <div style="width:100px;border-radius:12px;overflow:hidden;border:2px solid ${isChamp?'#f59e0b':r.border};box-shadow:0 0 ${isChamp?'16px rgba(245,158,11,0.7)':'10px '+r.glow};background:#111827;cursor:pointer;position:relative" title="${c.name}${isChamp?' ♛ Champion':''}">
...
function addToDeck(name) {
    ...
    // Champion: มีได้แค่ 1 ใบในเด็ค
    const cardData = (typeof CardSets !== 'undefined') ? (CardSets[deckBuilderState.theme]||{})[name] : null;
    if (cardData?.isChampion && (countMap[name]||0) >= 1) { showToast('♛ Champion มีได้แค่ 1 ใบในเด็ค!','#fbbf24'); return; }
...
function startRankedOnline(deckId) {
    ...
    _collectionDeckUsed = false;
    _isRankedGame = true;
    if (typeof selectedPlayerTheme !== 'undefined') selectedPlayerTheme = deck.theme;
    if (typeof gameMode !== 'undefined') gameMode = 'online';
    document.getElementById('hub-screen').style.display = 'none';
    document.getElementById('game-screen').style.display = '';
    const ts = document.getElementById('theme-selector');
    if (ts) {
        ...
        setTimeout(() => {
            const p2theme = document.getElementById('p2-online-theme');
            if (p2theme) p2theme.value = deck.theme;
        }, 100);
    }
}
