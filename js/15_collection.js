...
function showCardDetailModal(name, theme) {
    ...
    // Backdrop แยกต่างหาก — ครอบทั้งหน้าจอ
    const backdrop = document.createElement('div');
    backdrop.id = 'card-detail-backdrop';
    backdrop.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.88);z-index:99997;';
    backdrop.onclick = () => { backdrop.remove(); document.getElementById('card-detail-modal')?.remove(); };
    document.body.appendChild(backdrop);

    // Modal content — ใช้ left:50% + transform centering ที่ถูกต้องเสมอ
    const modal = document.createElement('div');
    modal.id = 'card-detail-modal';
    modal.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:99998;max-width:420px;width:90%;max-height:90vh;overflow-y:auto;overflow-x:hidden;box-sizing:border-box;display:block;';
    ...
    <div style="background:linear-gradient(135deg,#0f0c29,#1a1640,#1e1b4b);border:3px solid ${borderColor};border-radius:24px;
      ...
      <!-- Close button -->
      <button onclick="document.getElementById('card-detail-modal')?.remove();document.getElementById('card-detail-backdrop')?.remove()"
      ...
      <div style="padding:16px 18px">
        ...
        <div style="display:flex;gap:8px">
          <button onclick="sellCard('${name.replace(/'/g,"\\'")}','${theme}');document.getElementById('card-detail-modal')?.remove();document.getElementById('card-detail-backdrop')?.remove()"
            ${count <= 0 ? 'disabled' : ''}
