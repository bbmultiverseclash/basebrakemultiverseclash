...
async function loadPlayerDataFromFirebase(uid) {
    ...
    try {
        ...
        // ── [NEW] Login Reward: 2000 coins + 5 gems (ครั้งเดียวต่อบัญชี) ──
        if (!playerData.loginRewardClaimed) {
            playerData.loginRewardClaimed = true;
            playerData.coins = (playerData.coins || 0) + 2000;
            playerData.gems  = (playerData.gems  || 0) + 5;
            saveData();
            if (typeof showToast === 'function')
                showToast('🎁 Login Reward! +2000 🪙 +5 💎', '#fbbf24');
            // แสดง popup
            setTimeout(() => {
                const overlay = document.createElement('div');
                overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:10200;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.3s ease';
                overlay.innerHTML = `
                <div style="background:linear-gradient(135deg,#1c1640,#2d2060);border:3px solid #fbbf24;border-radius:24px;padding:32px 28px;max-width:360px;width:92%;text-align:center;box-shadow:0 0 80px rgba(251,191,36,0.5)">
                  <div style="font-size:3rem;margin-bottom:8px">🎁</div>
                  <div style="font-size:1.3rem;font-weight:900;color:#fbbf24;margin-bottom:6px">Login Reward!</div>
                  <div style="font-size:0.85rem;color:#9ca3af;margin-bottom:20px">ขอบคุณที่ Login ด้วย Google!</div>
                  <div style="display:flex;justify-content:center;gap:16px;margin-bottom:24px">
                    <div style="background:#111827;border:2px solid #fbbf24;border-radius:14px;padding:14px 20px">
                      <div style="font-size:1.6rem">🪙</div>
                      <div style="font-size:1.4rem;font-weight:900;color:#fcd34d">+2,000</div>
                      <div style="font-size:0.7rem;color:#9ca3af">Coins</div>
                    </div>
                    <div style="background:#111827;border:2px solid #60a5fa;border-radius:14px;padding:14px 20px">
                      <div style="font-size:1.6rem">💎</div>
                      <div style="font-size:1.4rem;font-weight:900;color:#93c5fd">+5</div>
                      <div style="font-size:0.7rem;color:#9ca3af">Gems</div>
                    </div>
                  </div>
                  <button onclick="this.closest('div[style*=fixed]').remove();if(typeof updateHubUI==='function')updateHubUI();"
                    style="background:linear-gradient(135deg,#d97706,#b45309);color:white;border:none;padding:12px 36px;border-radius:14px;font-weight:900;font-size:1rem;cursor:pointer;box-shadow:0 0 20px rgba(217,119,6,0.4)">
                    🎉 รับเลย!
                  </button>
                </div>`;
...
function redeemCode() {
    ...
    if (reward.easterTokens) playerData.easterTokens  = (playerData.easterTokens || 0) + reward.easterTokens;
    saveData();
    markCodeUsed(raw);
    updateHubUI();
    const parts = [];
