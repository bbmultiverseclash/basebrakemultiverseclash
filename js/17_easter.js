...
// [NEW] บังคับเขียนทับรูปใน CardSets ทุกครั้งที่โหลดเกม — ทำให้รูปค้างถาวร
function applyGlobalArtstyles() {
    if (!playerData.equippedArtstyles || typeof CardSets === 'undefined') return;
    Object.entries(playerData.equippedArtstyles).forEach(([cardName, styleId]) => {
        const style = ARTSTYLE_CFG[styleId];
        if (style) {
            Object.values(CardSets).forEach(set => {
                if (set && set[cardName]) set[cardName].art = style.art;
            });
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 13. Patch endGame → award Easter Token on win (ถาวร) + Rabbit Rampant (event เท่านั้น)
    ...
    if (typeof _origEndGame === 'function') {
        window.endGame = function(winner) {
            ...
            // เคลียร์ bg เสมอ (ไม่ว่าใครชนะ/event หมดหรือยัง)
            ...
            // ── Rabbit Rampant bonus (event เท่านั้น) ────────────────
            if (isEasterActive() && _isRabbitRampantGame) {
            ...
            // ── Token ปกติ — ได้ถาวรแม้ event หมด ───────────────────
            if (isAI || isRanked || isOnline) {
                if (!playerData.easterTokens) playerData.easterTokens = 0;
                playerData.easterTokens++;
                saveData();
                setTimeout(() => showToast('🐣 +1 Easter Token', '#86efac'), 1200);
            }
        };
    }
    ...
    if (typeof _origStartRanked === 'function') {
        window.startRankedGame = function(deckId) {
            ...
            if (isEasterActive() && Math.random() < 0.30) {
                // [FIX] ไม่ set _isRabbitRampantGame ที่นี่ — ให้ buildDeck('easter') เป็นคนกำหนด
            ...
            // [FIX] reset flag — ถ้า buildDeck ถูกเรียกด้วย theme อื่น มันจะ reset เองอยู่แล้ว
            _isRabbitRampantGame = false;
    ...
    // [FIX] _isRabbitRampantGame ถูก set ที่นี่เท่านั้น — ต้อง build deck easter จริงๆ ถึงจะได้ bonus
    const _origBuildDeck = window.buildDeck;
    if (typeof _origBuildDeck === 'function') {
        window.buildDeck = function(theme) {
            if (theme === 'easter') {
                // ยืนยัน flag ตรงนี้ — AI deck เป็น 65 Great Rabbits จริงๆ
            ...
            // theme อื่นๆ → ไม่ใช่ Rabbit Rampant — reset flag ทันที
            _isRabbitRampantGame = false;
    ...
    _initArtstyles();
    // [NEW] บังคับ apply artstyle ทั้งหมดลง CardSets เพื่อให้รูปค้างถาวร
    setTimeout(applyGlobalArtstyles, 500);
