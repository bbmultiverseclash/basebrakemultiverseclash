...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof CardSets !== 'undefined') {
        Object.keys(_ULTRA_CARDS).forEach(k => {
            ...
            const cardCopy = JSON.parse(JSON.stringify(_ULTRA_CARDS[k]));
            cardCopy.isUltra = true;
            CardSets[theme][k] = cardCopy;
        });

        // Patch กรอง Ultra Cards ออกจาก pack ปกติ
        if (typeof getSetCardPool === 'function') {
            const _origGetPool = window.getSetCardPool;
            window.getSetCardPool = function(setName) {
                return _origGetPool(setName).filter(c => !c.data?.isUltra);
            };
        }

        // Patch Ready Pack ด้วย
        if (typeof openReadyPack === 'function') {
            const _origReadyPack = window.openReadyPack;
            window.openReadyPack = function() {
                const hidden = {};
                Object.keys(_ULTRA_CARDS).forEach(k => {
                    const theme = _ULTRA_CARDS[k]._theme;
                    if (CardSets[theme]?.[k]) {
                        hidden[`${k}|${theme}`] = CardSets[theme][k];
                        delete CardSets[theme][k];
                    }
                });
                _origReadyPack.apply(this, arguments);
                Object.entries(hidden).forEach(([key, data]) => {
                    const [name, theme] = key.split('|');
                    if (!CardSets[theme]) CardSets[theme] = {};
                    CardSets[theme][name] = data;
                });
            };
        }
    }
    ...
    if (typeof REDEEM_CODES !== 'undefined') {
        ...
        REDEEM_CODES['IWIWURZW']   = { hxhCard: 'megumin', label: '💥🐋 megumin + White Whale', oneTime: true };
    }

    // ─── PATCH: IWIWURZW gives both megumin AND White Whale ──────
    // ลอง hook หลาย function name เพราะเกม main file อาจใช้ชื่อต่าง ๆ
    (['redeemCode','submitRedeemCode','applyCode','handleRedeemCode']).forEach(fnName => {
        if (typeof window[fnName] === 'function') {
            const _orig_redeem_iwiwurzw = window[fnName];
            window[fnName] = function() {
                const beforeUsed = (playerData.usedCodes && Array.isArray(playerData.usedCodes))
                    ? [...playerData.usedCodes] : [];
                _orig_redeem_iwiwurzw.apply(this, arguments);
                const afterUsed = playerData.usedCodes || [];
                // ถ้า IWIWURZW เพิ่งถูก redeem ครั้งนี้ → ให้ White Whale เพิ่ม
                if (!beforeUsed.includes('IWIWURZW') && afterUsed.includes('IWIWURZW')) {
                    const wwKey = 'White Whale|isekai_adventure';
                    playerData.collection[wwKey] = (playerData.collection[wwKey] || 0) + 1;
                    if (typeof showToast === 'function') showToast(`🐋 ได้รับการ์ดพิเศษเพิ่ม: White Whale!`, '#38bdf8');
    ...
    if (typeof renderPacksPanel === 'function') {
        ...
        REDEEM_CODES['KJ99MGM1'] = { ultraCard: 'megumin', theme: 'isekai_adventure', label: '💥 megumin' };
        REDEEM_CODES['PL00WHAL'] = { ultraCard: 'White Whale', theme: 'isekai_adventure', label: '🐋 White Whale' };
    }
