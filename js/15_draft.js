...
// ── Patch selectMode to handle 'draft' ──────────────────────
const _origSelectMode = window.selectMode;
window.selectMode = function(mode) {
    // Highlight Draft button
    const draftBtn = document.getElementById('btn-mode-draft');
    if (draftBtn) draftBtn.style.border = '2px solid transparent';

    if (mode === 'draft') {
        // Highlight draft button, show online panel (for Room ID input), hide theme pickers
        ['btn-mode-ai','btn-mode-local','btn-mode-online','btn-mode-chaos'].forEach(id => {
            const b = document.getElementById(id);
            if (b) b.style.border = '2px solid transparent';
        });
        if (draftBtn) draftBtn.style.border = '2px solid #a855f7';

        gameMode = 'draft';

        // Show online panel (Room ID input), hide P2 theme div
        const op = document.getElementById('online-panel');
        if (op) op.style.display = 'flex';
        const p2d = document.getElementById('p2-theme-div');
        if (p2d) p2d.style.display = 'none';
        const p2online = document.getElementById('p2-theme-online');
        if (p2online) p2online.style.display = 'none';

        // Change start button label
        const startBtn = document.getElementById('start-game-btn');
        if (startBtn) {
            startBtn.style.background = 'linear-gradient(135deg,#7c3aed,#db2777)';
            startBtn.textContent = '⚔️ เริ่ม Draft Duel';
        }
        return;
    }

    // Restore start button if switching away from draft
    if (gameMode === 'draft') {
        const startBtn = document.getElementById('start-game-btn');
        if (startBtn) {
        ...
        const p2d = document.getElementById('p2-theme-div');
        if (p2d) p2d.style.display = '';
    }

    if (typeof _origSelectMode === 'function') _origSelectMode(mode);
};

// ── Patch start-game-btn to handle draft mode ────────────────
// We hook AFTER 11_mulligan.js sets its onclick, so we wrap it
document.addEventListener('DOMContentLoaded', () => {
    ...
    startBtn.onclick = async function() {
        ...
        if (typeof origOnclick === 'function') origOnclick.call(this);
    };
});
