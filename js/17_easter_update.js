...
;(function _easterInit() {
    ...
    setTimeout(() => {
        if (typeof renderHomePanel==='function') {
            ...
            window.renderHomePanel = function() {
                _orig();
                setTimeout(_injectEasterHomeButtons, 0);
            };
        }
    },0);
    ...
    setTimeout(_addPaletteNavButton, 50);

    // Init playerData immediately
    if (typeof playerData!=='undefined') _initEasterPlayerData();
    _injectEasterRedeemCodes();
    _injectEasterTitle();

    // Re-render hub after all patches applied
    setTimeout(() => {
        if (typeof showHubTab === 'function') showHubTab('home');
    }, 100);
})();
