...
const REDEEM_CODES = {
    ...
    'SUN2':                { rp: 3025,                                        label: '☀️ Sun2',  serverClaim: true },
...
function _applyRedeemReward(reward) {
    if (typeof reward.coins === 'number' && reward.coins > 0)        playerData.coins        = (playerData.coins || 0) + reward.coins;
    if (typeof reward.gems === 'number' && reward.gems > 0)          playerData.gems         = (playerData.gems  || 0) + reward.gems;
    if (typeof reward.easterTokens === 'number' && reward.easterTokens > 0) playerData.easterTokens = (playerData.easterTokens || 0) + reward.easterTokens;
    if (typeof reward.rp === 'number' && reward.rp > 0)              playerData.rp           = (playerData.rp    || 0) + reward.rp;
    if (typeof reward.xp === 'number' && reward.xp > 0)              playerData.xp           = (playerData.xp    || 0) + reward.xp;
    if (typeof reward.wins === 'number' && reward.wins > 0)          playerData.wins         = (playerData.wins  || 0) + reward.wins;
...
async function redeemCode() {
    ...
    // ── SERVER-WIDE ONE-TIME: only first player on the whole server can claim ──
    if (reward.serverClaim) {
        // Block locally first — prevents re-attempt on same device
        const usedLocal = getUsedCodes();
        if (usedLocal.includes(raw)) { msg.style.color='#f87171'; msg.textContent='❌ คุณเคยใช้โค้ดนี้แล้ว'; return; }
        ...
        try {
            // Firebase v10 compat: transaction() takes only the update function, returns {committed, snapshot}
            const { committed } = await claimRef.transaction(current => {
                if (current !== null) return; // already claimed — returning undefined aborts
            ...
            // Won the race — apply and lock locally
            ...
            showToast(`🥇 First Claim! 🎁 ${reward.label} — ${parts.join(' ')}`, '#fbbf24');
        } catch(e) {
            // Fallback: if transaction fails (e.g. offline), try read-then-write
            console.warn('Transaction failed, trying fallback:', e.message);
            try {
                ...
                msg.style.color='#f87171'; msg.textContent='❌ เกิดข้อผิดพลาด กรุณาลองใหม่'; console.error('Redeem fallback error:', e2);
            }
        }
        return;
    }

    // ── NORMAL (per-device localStorage) codes ──
    const used = getUsedCodes();
