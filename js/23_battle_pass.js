...
// ============================================================
// SUNSK VALUE BUNDLE (50 Gems = Sunsk + Katakuri Artstyle + 10000 BP)
// วางไว้ล่างสุดของไฟล์ 23_battle_pass.js
// ============================================================
const SUNSK_EXPIRY = new Date('2026-05-15T00:00:00+07:00').getTime(); // ปรับวันหมดอายุได้ตามต้องการ

document.addEventListener('DOMContentLoaded', () => {
    // 1. เพิ่มการ์ด Sunsk (ห้ามสุ่มออกกล่องปกติด้วย shopOnly: true)
    if (typeof CardSets !== 'undefined') {
        if (!CardSets['suankularb']) CardSets['suankularb'] = {};
        CardSets['suankularb']['Sunsk'] = {
            name: 'Sunsk', type: 'Character', cost: 5, atk: 3, hp: 4, maxHp: 4,
            text: 'โจมตีได้ 2 ครั้งต่อเทิร์น', color: 'bg-orange-600', maxAttacks: 2,
            art: 'https://file.garden/aeeLCXSsJxTPrRbp/file_0000000069dc720cae2d6057a64c0150.png',
            _theme: 'suankularb', shopOnly: true 
        };
    }

    // 2. เพิ่ม Artstyle Pongneng (Katakuri)
    if (typeof window.ARTSTYLE_CFG === 'undefined') window.ARTSTYLE_CFG = {};
    window.ARTSTYLE_CFG['pongneng_katakuri'] = {
        id: 'pongneng_katakuri', label: 'Pongneng - Katakuri', emoji: '🍩',
        targetCard: 'Pongneng', art: 'https://file.garden/aeeLCXSsJxTPrRbp/file_000000004fd8720c884451ff27da8ce2.png',
        shopCost: 0, isBPExclusive: true
    };

    // 3. เตรียมตัวแปรเซฟ
    if (typeof playerData !== 'undefined') {
        if (playerData.sunskPackBought === undefined) playerData.sunskPackBought = false;
    }

    // 4. แทรก UI ลงใน renderBattlePass เดิมแบบไดนามิก
    if (typeof window.renderBattlePass === 'function') {
        const _origRenderBP = window.renderBattlePass;
        window.renderBattlePass = function() {
            _origRenderBP.apply(this, arguments); // เรียก UI ปกติให้วาดก่อน
            
            const pnl = document.getElementById('hub-panel-bp');
            if (!pnl) return;
            
            const wrapper = pnl.querySelector('div[style*="max-width: 500px"]');
            if (!wrapper) return;

            // ตรวจสอบว่าเคยใส่กล่องนี้ไปแล้วหรือยัง
            if (document.getElementById('sunsk-bundle-ui')) return;

            const isSunskAvailable = Date.now() < SUNSK_EXPIRY;
            const sunskBought = playerData.sunskPackBought || false;
            // เช็คว่ามี Gem พอไหม (รองรับทั้งระบบรวม limitedGem และ gem ธรรมดา)
            const totalGems = typeof getTotalGems === 'function' ? getTotalGems() : (playerData.gems || 0);
            const canBuySunsk = isSunskAvailable && !sunskBought && totalGems >= 50;

            const sunskHtml = `
            <div id="sunsk-bundle-ui" style="background: linear-gradient(135deg, #431407, #000000); border: 3px solid ${isSunskAvailable ? '#f97316' : '#374151'}; border-radius: 24px; padding: 20px; margin-bottom: 20px; box-shadow: 0 10px 30px rgba(249,115,22,0.3); position: relative; overflow: hidden; display: flex; gap: 20px; align-items: center;">
                <div style="position: absolute; top: 15px; right: -30px; background: ${isSunskAvailable ? '#ef4444' : '#374151'}; color: white; padding: 5px 40px; font-weight: 900; font-size: 0.7rem; transform: rotate(45deg);">
                    ${isSunskAvailable ? 'LIMITED' : 'EXPIRED'}
                </div>
                <div style="width: 80px; height: 110px; border-radius: 10px; border: 2px solid #f97316; overflow: hidden; flex-shrink: 0; box-shadow: 0 0 15px rgba(249,115,22,0.4);">
                    <img src="https://file.garden/aeeLCXSsJxTPrRbp/file_0000000069dc720cae2d6057a64c0150.png" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
                <div style="flex: 1;">
                    <div style="color: #f97316; font-weight: 900; font-size: 0.7rem; letter-spacing: 2px;">☀️ BP BOOSTER BUNDLE</div>
                    <h3 style="margin: 4px 0; font-size: 1.2rem; font-weight: 900; color: white;">Sunsk & Katakuri</h3>
                    <div style="font-size: 0.75rem; color: #fdba74; margin-bottom: 10px; line-height: 1.4;">
                        การ์ด <strong style="color:#f97316;">Sunsk</strong> + สกิน <strong style="color:#f97316;">Pongneng (Katakuri)</strong> + <strong style="color:#fbbf24;">10,000 BP</strong>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <div style="font-size: 1.1rem; font-weight: 900; color: ${isSunskAvailable ? '#f97316' : '#6b7280'};">💎 50 Gems</div>
                        <button onclick="buySunskBPPack()" ${canBuySunsk ? '' : 'disabled'} style="background: ${canBuySunsk ? 'linear-gradient(135deg, #ea580c, #9a3412)' : '#374151'}; color: ${canBuySunsk ? 'white' : '#9ca3af'}; border: none; padding: 8px 18px; border-radius: 10px; font-weight: 900; cursor: ${canBuySunsk ? 'pointer' : 'not-allowed'}; font-size: 0.8rem;">
                            ${sunskBought ? '✅ Purchased' : !isSunskAvailable ? '🔒 Expired' : totalGems < 50 ? '💎 Not Enough' : 'BUY NOW'}
                        </button>
                    </div>
                </div>
            </div>`;
            
            // แทรกไว้บนสุดของกล่อง (ก่อน Fern/Trigon Bundle)
            wrapper.insertAdjacentHTML('afterbegin', sunskHtml);
        };
    }
});

// 5. ฟังก์ชันสำหรับกดซื้อ Bundle นี้
window.buySunskBPPack = function() {
    if (Date.now() > SUNSK_EXPIRY) { if (typeof showToast === 'function') showToast('❌ แพ็กเกจนี้หมดเวลาแล้ว!', '#f87171'); return; }
    if (playerData.sunskPackBought) { if (typeof showToast === 'function') showToast('✅ คุณเป็นเจ้าของแพ็กนี้แล้ว', '#4ade80'); return; }
    
    // คำนวณรวม Gem (รองรับ Limited Gems ถ้ามี)
    const totalGems = typeof getTotalGems === 'function' ? getTotalGems() : (playerData.gems || 0);
    if (totalGems < 50) { if (typeof showToast === 'function') showToast('💎 Gems ไม่พอ!', '#f87171'); return; }

    // หักเพชร
    if (typeof spendGems === 'function') spendGems(50);
    else playerData.gems -= 50;

    playerData.sunskPackBought = true;

    // มอบการ์ด Sunsk
    const cardKey = 'Sunsk|suankularb';
    playerData.collection[cardKey] = (playerData.collection[cardKey] || 0) + 1;

    // มอบ Artstyle สกิน
    if (!playerData.unlockedArtstyles) playerData.unlockedArtstyles =
[];
    if (!playerData.unlockedArtstyles.includes('pongneng_katakuri')) {
        playerData.unlockedArtstyles.push('pongneng_katakuri');
    }

    // มอบ 10,000 BP
    playerData.bp = (playerData.bp || 0) + 10000;

    // บันทึกข้อมูล
    if (typeof savePlayerData === 'function') savePlayerData();

    // แสดง Toast + รีเรนเดอร์
    if (typeof showToast === 'function') showToast('🎉 ได้รับ Sunsk + Katakuri Artstyle + 10,000 BP!', '#f97316');
