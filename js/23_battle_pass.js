...
// ============================================================
// HOTFIX: เพิ่ม Artstyle จาก JJK Box ลงในระบบให้มองเห็น + กดใส่ได้
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    if (typeof window.ARTSTYLE_CFG === 'undefined') window.ARTSTYLE_CFG = {};
    ...
    // เพิ่มเข้า Store และตั้งค่าเป็นของจาก Battle Pass / กล่องสุ่ม
    missingArts.forEach(a => {
        window.ARTSTYLE_CFG[a.id] = { ...a, isBPExclusive: true, shopCost: 0 };
    });

    // มาร์ค Artstyle ของ JJK อันอื่นว่าเป็นของ BP ด้วย จะได้ไม่บัคโชว์ปุ่มซื้อ 0 Gem
    ['megumi_jp', 'toji_wrath', 'mahoraga_mafia'].forEach(id => {
        if (window.ARTSTYLE_CFG[id]) window.ARTSTYLE_CFG[id].isBPExclusive = true;
    });

    // 2. ดักจับหน้าต่าง Artstyle เพื่อเปลี่ยนปุ่ม "ซื้อ" เป็น "ล็อค" สำหรับคนที่สุ่มยังไม่ได้
        ...
        window.renderUnifiedArtstyleShop = function() {
            ...
            buttons.forEach(btn => {
                ...
                if (onclick.includes('buyUnifiedArtstyle') || onclick.includes('buyArtstyle')) {
                    ...
                    if (match) {
                        const id = match[1];
                        const cfg = window.ARTSTYLE_CFG[id];
