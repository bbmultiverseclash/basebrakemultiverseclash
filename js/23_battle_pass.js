...
// ─── Inject Tab + Panel (ใช้โครงสร้างเดียวกับ BP tab) ────────
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (navBar && !document.getElementById('hub-tab-quests')) {
        ...
        btn.onclick = () => showHubTab('quests');
        navBar.appendChild(btn);
    }

    // 2. หน้าจอ Quests Panel
    const container = document.getElementById('hub-panel-home')?.parentElement;
    ...
    // 3. เชื่อมต่อระบบ Tab (chain ต่อจาก showHubTab ที่ BP patch ไว้แล้ว)
    const _origTabQ = window.showHubTab;
    window.showHubTab = function(tab) {
        const qBtn = document.getElementById('hub-tab-quests');
        const qPnl = document.getElementById('hub-panel-quests');
        if (tab === 'quests') {
            ...
            if (qBtn) qBtn.classList.add('active-tab');
            if (qPnl) { qPnl.style.display = 'block'; renderQuestPanel(); }
        } else {
            if (qBtn) qBtn.classList.remove('active-tab');
            if (qPnl) qPnl.style.display = 'none';
            if (_origTabQ) _origTabQ(tab);
        }
    };

    // 4. Auto-patch game events
    if (typeof window.awardWin === 'function') {
