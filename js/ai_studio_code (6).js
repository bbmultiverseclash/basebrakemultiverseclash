...
function renderBattlePass() {
    ...
    for (let i = 1; i <= 20; i++) {
        ...
        rows += `
        <div style="display: flex; align-items: center; gap: 10px; background: ${isReached ? 'rgba(251,191,36,0.1)' : '#111827'}; border: 1px solid ${isReached ? '#fbbf24' : '#374151'}; padding: 12px; border-radius: 12px; margin-bottom: 8px;">
            <div style="width: 30px; font-weight: 900; color: ${isReached ? '#fbbf24' : '#4b5563'}; text-align: center;">${i}</div>
            ...
            <div style="flex: 1; font-size: 0.75rem; position: relative;">
                ${!playerData.hasPremiumBP ? '<div style="position:absolute;inset:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:2;border-radius:4px;">🔒</div>' : ''}
...
function renderBPItem(item, pool, level, isReached) {
    if (!item) return `<span style="color:#374151">—</span>`;
    ...
    let label = "";
    if (item.type === 'card') label = `🃏 ${item.name}`;
    else if (item.type === 'coins') label = `🪙 ${item.amount}`;
    else if (item.type === 'gems') label = `💎 ${item.amount}`;
    else if (item.type === 'xp') label = `⭐ ${item.amount} XP`;
    else if (item.type === 'avatar') label = `👤 ${item.label}`;
    else if (item.type === 'banner') label = `🖼️ ${item.label}`;
    else if (item.type === 'multi') label = `🎁 Multi Reward`;

    return `
        <div style="color: ${isClaimed ? '#4ade80' : 'white'}; font-weight: bold;">
            ${label}
            <div style="margin-top: 4px;">
                ${isClaimed ? '✅' : canClaim ? `<button onclick="claimBP('${pool}', ${level})" style="background:#fbbf24; border:none; border-radius:4px; font-size:0.6rem; padding:2px 8px; font-weight:900; cursor:pointer;">CLAIM</button>` : '🔒'}
            </div>
        </div>`;
}
