...
function _buildHxHRankedDeck() {
    ...
    const makeCard = (name, theme, costOverride, atkBonus, hpBonus) => {
        const c = typeof createCardInstance === 'function'
            ? createCardInstance(name, theme)
            : null;
        if (!c) return null;
        if (costOverride !== undefined) c.cost = costOverride;
        if (atkBonus) { c.atk = (c.atk || 0) + atkBonus; }
        if (hpBonus)  { c.hp  = (c.hp  || 0) + hpBonus; c.maxHp = (c.maxHp || 0) + hpBonus; }
        return c;
    };

    // 30x Dark Continent Monster — cost 5, pre-buffed +5 ATK/HP (field card effect)
    for (let i = 0; i < 30; i++) {
        const c = makeCard('Dark Continent Monster', 'hxh', 5, 5, 5);
        if (c) deck.push(c);
    }
    // 20x Chimera Ant — cost 6
    for (let i = 0; i < 20; i++) {
    ...
    for (let i = 0; i < 6; i++) {
        const c = typeof createCardInstance === 'function'
            ? createCardInstance('Dark Continent', 'hxh')
            : null;
        if (c) { c.cost = 4; deck.push(c); }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof buildDeck === 'function') {
        ...
        window.buildDeck = function(theme) {
            ...
            if (theme !== 'easter') _isHxHRankedGame = false;
            return _origBuildDeckHxH.call(this, theme);
        };
    }

    // ── V2h. Patch startRankedGame → 50% HxH Ranked Challenge ──
    if (typeof startRankedGame === 'function') {
        ...
        window.startRankedGame = function(deckId) {
            if (Math.random() < 0.30) {
                ...
                if (typeof selectedAITheme !== 'undefined') selectedAITheme = 'hxh_ranked';
                // Set Dark Continent as shared field card + apply BG
                setTimeout(() => {
                    if (typeof state !== 'undefined') {
                        const dcCard = JSON.parse(JSON.stringify(_DC_FIELD_CARD_TPL));
                        dcCard.id = 'dc_field_' + Date.now();
                        state.sharedFieldCard      = dcCard;
                        state.sharedFieldCardOwner = 'ai';
                    }
                    _applyHxHRankedBG();
                    if (typeof updateUI === 'function') updateUI();
                    if (typeof log === 'function')
                        log('🌑 [Dark Continent] Field Card active! Dark Continent Monster ทั้งหมด +5 ATK/HP', 'text-slate-300 font-bold');
                }, 400);
                return;
            }
    ...
    // ── V2i. Patch endGame → HxH Ranked win reward ─────────────
    if (typeof endGame === 'function') {
        ...
        window.endGame = function(winner) {
            ...
            // ── Regular ranked win: +1 Rod Token + 3% Yoyo ──────────
            if (winner === 'player' && typeof gameMode !== 'undefined' && gameMode === 'ai') {
                if (typeof playerData !== 'undefined') {
                    playerData.rodTokens = (playerData.rodTokens || 0) + 1;
                    if (typeof showToast === 'function')
                        setTimeout(() => showToast('+1 🎣 Rod Token', '#fcd34d'), 800);
                    if (Math.random() < 0.03) {
                        const key = 'Yoyo|hxh';
                        playerData.collection[key] = (playerData.collection[key] || 0) + 1;
                        if (typeof showToast === 'function')
                            setTimeout(() => showToast('🪀 Yoyo Dropped!', '#38bdf8'), 1400);
                    }
                    if (typeof saveData === 'function') saveData();
                }
            }
            // ── Normal ranked win → +1 Rod Token + 3% Yoyo ────────
            if (winner === 'player' && typeof gameMode !== 'undefined' && gameMode !== 'online') {
                if (typeof playerData !== 'undefined') {
                    playerData.rodTokens = (playerData.rodTokens || 0) + 1;
                    if (typeof showToast === 'function')
                        setTimeout(() => showToast('+1 🎣 Rod Token', '#fcd34d'), 800);
            ...
            if (winner === 'player' && typeof gameMode !== 'undefined' && gameMode === 'ai') {
                if (typeof playerData !== 'undefined') {
                    ...
                    if (Math.random() < 0.03) {
                        const key = 'Yoyo|hxh';
                        playerData.collection[key] = (playerData.collection[key] || 0) + 1;
                        setTimeout(() => { if (typeof showToast === 'function') showToast('🪀 Yoyo Dropped!', '#38bdf8'); }, 1600);
                    }
                    if (typeof saveData === 'function') saveData();
                    setTimeout(() => { if (typeof showToast === 'function') showToast('+1 🎣 Rod Token', '#fcd34d'); }, 400);
                }
            }
        };
    }
