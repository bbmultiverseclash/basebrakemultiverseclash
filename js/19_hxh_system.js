...
const _HXH_CARDS_TPL = {
    ...
    'Kurapika': {
        ...
        text:'On Summon: ศัตรู HP สูงสุด → HP = 1 + Paralyze 2T | On Death: Paralyze ศัตรู 2 ตัวสุ่ม (2T)',
...
function buyRodShopItem(key) {
    ...
    if (owned >= cfg.maxOwn) {
        showToast(`🔒 มีครบ ${cfg.maxOwn} ใบแล้ว!`, '#f87171'); return;
    }
    ...
    updateHubUI();
    checkTitleUnlocks();
    showToast(`✅ ได้รับ "${key}"! (${playerData.rodTokens} 🎣 เหลือ)`, '#4ade80');
    renderRodShop(); // refresh shop UI
}
...
function renderRodShop() {
    ...
    overlay.id = '_rod-shop-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:3000;display:flex;align-items:center;justify-content:center;padding:12px;overflow-y:auto';
    ...
    const rows = ROD_SHOP_ITEMS.map(cfg => {
        ...
        const catColor = cfg.category === 'character' ? '#86efac'
                       : cfg.category === 'item'      ? '#fcd34d'
                       : '#f9a8d4';
        return `
        <div style="background:#0f172a;border:1.5px solid ${canBuy?'#fcd34d':'#374151'};
             border-radius:14px;padding:12px;display:flex;align-items:center;gap:10px">
          ${tpl.art ? `<img src="${tpl.art}" style="width:56px;height:68px;object-fit:cover;border-radius:8px;border:1px solid #374151" onerror="this.style.display='none'">` : `<div style="width:56px;height:68px;background:#1f2937;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:1.4rem">🩹</div>`}
          <div style="flex:1;min-width:0">
            <div style="font-size:0.62rem;font-weight:900;color:${catColor};text-transform:uppercase;letter-spacing:0.5px">${cfg.category}</div>
            ...
            <div style="font-size:0.65rem;color:#6b7280;margin-top:2px">Cost ${tpl.cost}${tpl.atk ? ` · ATK ${tpl.atk}` : ''}${tpl.hp ? ` · HP ${tpl.hp}` : ''} ${cfg.untradeable ? '· 🔒 Untradeable' : ''}</div>
            <div style="font-size:0.62rem;margin-top:3px;color:${owned>=cfg.maxOwn?'#4ade80':'#d1d5db'}">มี: ${owned}/${cfg.maxOwn}</div>
          </div>
          <button onclick="buyRodShopItem('${cfg.key}')"
            ${canBuy ? '' : 'disabled'}
            style="background:${canBuy ? 'linear-gradient(135deg,#d97706,#92400e)' : '#374151'};
                   color:${canBuy ? 'white' : '#6b7280'};border:none;
                   padding:9px 13px;border-radius:10px;font-weight:900;
                   font-size:0.8rem;cursor:${canBuy?'pointer':'not-allowed'};
    ...
    overlay.innerHTML = `
    <div style="background:linear-gradient(135deg,#0a0f1e,#1a0a2e);
         border:2.5px solid #fcd34d;border-radius:24px;padding:24px 18px;
         max-width:440px;width:100%;max-height:90vh;overflow-y:auto;
         box-shadow:0 0 60px rgba(252,211,77,0.25)">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div>
          <div style="font-size:1.25rem;font-weight:900;color:#fcd34d">🎣 Fishing Rod Shop</div>
          <div style="font-size:0.68rem;color:#9ca3af">ซื้อการ์ด HxH พิเศษด้วย Rod Token</div>
        </div>
        <div style="background:#1f2937;border:1px solid #fcd34d;border-radius:12px;padding:8px 12px;text-align:center">
          <div style="font-size:0.6rem;color:#9ca3af">Rod Token</div>
          <div style="font-size:1.1rem;font-weight:900;color:#fcd34d">${rod} 🎣</div>
        </div>
      </div>
      <div style="font-size:0.62rem;color:#6b7280;margin-bottom:12px;background:rgba(252,211,77,0.06);border:1px solid #92400e55;border-radius:8px;padding:6px 10px;text-align:center">
        🏆 ชนะ AI = +1 🎣 | 3% โอกาสได้ 🪀 Yoyo | ใช้ 60 🎣 → 👑 Haunted Hunter
      </div>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${rows}
      </div>
      <button onclick="document.getElementById('_rod-shop-overlay').remove()"
        style="width:100%;margin-top:16px;background:#374151;color:#9ca3af;
               border:none;padding:11px;border-radius:12px;font-weight:700;
               font-size:0.9rem;cursor:pointer">✕ ปิด</button>
    </div>`;

    const old = document.getElementById('_rod-shop-overlay');
    if (old) old.remove();
...
<div style="background:rgba(251,191,36,0.06);border:1px solid #92400e55;
  ...
  🎣 <strong>Fishing Rod Token</strong> — ชนะ AI ได้ +1 Rod ต่อเกม! มี ${rod} Rod
</div>
...
function _patchHxHRodDisplay() {
    ...
    if (el) { el.textContent = `🎣 ${rod}`; return; }
    ...
    div.id = '_hub-hxh-rod';
    div.style.cssText = 'font-size:0.85rem;font-weight:800;color:#fcd34d';
    div.textContent = `🎣 ${rod}`;
    ...
    if (eff === 'Kurapika' && !card._kurapikaOS) {
        ...
        if (enemies.length > 0) {
            ...
            if (typeof log === 'function')
                log(`⛓️ [Kurapika] Judgement Chain! ${topHP.name} HP → 1 + Paralyze 2T!`, 'text-red-400 font-bold');
        }
    }
...
// ─── ROD SHOP CARD MECHANICS ─────────────────────────────────

// Chimera Ant ongoing: +2 ATK per Chimera Ant in own field
function _hxhChimeraAntOngoing(playerKey) {
    if (typeof state === 'undefined') return;
    const p = state.players[playerKey];
    if (!p) return;
    const antCount = p.field.filter(c => (c.originalName || c.name) === 'Chimera Ant').length;
    p.field.forEach(c => {
        if ((c.originalName || c.name) !== 'Chimera Ant') return;
        const base = 3;
        c.atk = base + (antCount * 2);
    });
}

// Hunter License on kill: draw 1 + ATK +2 permanently
function _hxhHunterLicenseOnKill(equippedChar, playerKey) {
    if (typeof state === 'undefined') return;
    const p = state.players[playerKey];
    if (!p) return;
    const char = p.field.find(c => c.id === equippedChar.id);
    if (!char) return;
    // ATK +2 permanently
    char.atk = (char.atk || 0) + 2;
    // Draw 1 card
    if (p.deck.length > 0) {
        p.hand.push(p.deck.shift());
        if (typeof log === 'function')
            log(`📜 [Hunter License] Kill → ATK +2 (${char.atk}) + จั่ว 1!`, 'text-yellow-300 font-bold');
    }
}

// Fishing Rod on equip: 50% chance summon cost≤3 unit (100% if Gon)
function _hxhFishingRodOnEquip(equippedChar, playerKey) {
    if (typeof state === 'undefined') return;
    const p = state.players[playerKey];
    const eff = equippedChar.originalName || equippedChar.name;
    const isGon = eff === 'Gon';
    const chance = isGon ? 1.0 : 0.5;
    if (Math.random() > chance) {
        if (typeof log === 'function') log(`🎣 [Fishing Rod] Equipped — no summon this time`, 'text-cyan-300');
        return;
    }
    const maxSlots = typeof getMaxFieldSlots === 'function' ? getMaxFieldSlots(playerKey) : 6;
    if (p.field.length >= maxSlots) return;
    // Find cost≤3 card in deck
    const idx = p.deck.findIndex(c => (c.cost || 0) <= 3);
    if (idx === -1) return;
    const summoned = p.deck.splice(idx, 1)[0];
    summoned.attacksLeft = summoned.maxAttacks || 1;
    summoned.status = summoned.status || [];
    p.field.push(summoned);
    if (typeof log === 'function')
        log(`🎣 [Fishing Rod] ${isGon ? '100%' : '50%'} — Summon ${summoned.name} (Cost ${summoned.cost})!`, 'text-cyan-300 font-bold');
}

// Yoyo on Killua: grant maxAttacks:2 and mark for paralyze on attack
function _hxhYoyoEquipCheck(char) {
    const eff = char.originalName || char.name;
    const hasYoyo = (char.items || []).some(i => (i.originalName || i.name) === 'Yoyo');
    if (eff === 'Killua' && hasYoyo) {
        char.maxAttacks = 2;
        char._yoyoParalyze = true;
        if (typeof log === 'function')
            log(`🪀 [Yoyo] Killua gets 2 attacks + Paralyze on hit!`, 'text-sky-300 font-bold');
    }
}

// Yoyo paralyze on attack (called from postAttack)
function _hxhYoyoOnAttack(attacker, target) {
    if (!attacker._yoyoParalyze || !target) return;
    if (!target.status) target.status = [];
    if (!target.status.includes('Paralyze')) target.status.push('Paralyze');
    target.paralyzeTurns = Math.max(target.paralyzeTurns || 0, 2);
    if (typeof log === 'function')
        log(`🪀 [Yoyo] Paralyze → ${target.name}!`, 'text-sky-300');
}

// Bungee Gum: protect player base for 1 opponent turn
function _hxhPlayBungeeGum(playerKey) {
    if (typeof state === 'undefined') return;
    const p = state.players[playerKey];
    if (!p) return;
    p._bungeeGumActive = true;
    if (typeof log === 'function')
        log(`🩹 [Bungee Gum] Base ของ ${playerKey} ได้รับการป้องกัน 1 เทิร์น!`, 'text-pink-300 font-bold');
}
document.addEventListener('DOMContentLoaded', () => {

    // 1. Inject cards into CardSets (NOT in SETS_META → no pack dropdown entry)
    if (typeof CardSets !== 'undefined') {
        CardSets['hxh'] = {};
        Object.entries(_HXH_CARDS_TPL).forEach(([k, v]) => {
            CardSets['hxh'][k] = JSON.parse(JSON.stringify(v));
        });
        // Inject Rod Shop cards too
    ...
    if (typeof playerData !== 'undefined') {
        ...
        if (playerData.rodTokens       === undefined) playerData.rodTokens       = 0;
