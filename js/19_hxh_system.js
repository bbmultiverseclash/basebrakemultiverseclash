...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof buildDeck === 'function') {
        ...
        window.buildDeck = function(theme) {
            if (theme === 'hxh_ranked') {
                ...
                // [FIX] ยืนยัน flag ตรงนี้ เหมือน Easter ยืนยัน _isRabbitRampantGame ใน buildDeck
                // ป้องกันกรณี Easter แอบ set _isRabbitRampantGame ไว้ก่อน
    ...
    // [FIX] เหตุที่บักเดิม: 'hxh_ranked' ไม่ได้อยู่ใน SETS_META
    // base startRankedGame มันเลือก theme สุ่มจาก SETS_META เอง แล้ว override selectedAITheme ทิ้ง
    // → buildDeck('hxh_ranked') ไม่ถูกเรียกเลย แม้ HxH จะเซ็ต selectedAITheme ไว้แล้ว
    // วิธีแก้ตาม Easter: ชั่วคราวลบ theme ทั้งหมดออกจาก SETS_META แล้วใส่ 'hxh_ranked' ไว้อย่างเดียว
    // ทำให้ base ต้อง pick 'hxh_ranked' เสมอ แล้ว buildDeck จะถูกเรียกด้วย theme ที่ถูกต้อง
