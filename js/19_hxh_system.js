...
// LIMITED TIME (หมดเขต 27 เมษายน 2026 เที่ยงคืน)
//
// วางใน index.html ต่อจาก 18_theme_shop.js ก่อน </body>
// ============================================================

// ─── EXPIRY (27 เม.ย. 2026 เที่ยงคืน เวลาไทย — หมดหลัง 23:59 วันที่ 26) ───
const HXH_PACK_EXPIRY = new Date('2026-04-27T00:00:00+07:00');
...
function buyRodShopItem(key) {
    if (!_isHxHPackAvailable()) { showToast('🔒 HxH Event หมดแล้ว!', '#f87171'); return; }
...
function renderRodShop() {
    ...
    // ── ปิดแล้ว ────────────────────────────────────────────────
    if (!_isHxHPackAvailable()) {
        const rod = playerData.rodTokens || 0;
        ov.innerHTML = `
        <div style="background:linear-gradient(135deg,#0a0f1e,#1a0a2e);border:2.5px solid #374151;
             border-radius:24px;padding:28px 20px;max-width:400px;width:100%;text-align:center;
             box-shadow:0 0 40px rgba(0,0,0,0.6)">
          <div style="font-size:2.8rem;margin-bottom:8px">🎣</div>
          <div style="font-size:1.2rem;font-weight:900;color:#6b7280;margin-bottom:6px">Fishing Rod Shop</div>
          <div style="font-size:0.8rem;color:#f87171;font-weight:700;margin-bottom:12px">⏰ HxH Event สิ้นสุดแล้ว</div>
          <div style="background:#1f2937;border:1px solid #374151;border-radius:12px;padding:10px;margin-bottom:16px">
            <div style="font-size:0.7rem;color:#9ca3af;">Rod Token คงเหลือ</div>
            <div style="font-size:1.4rem;font-weight:900;color:#fcd34d">${rod} 🎣</div>
            <div style="font-size:0.62rem;color:#6b7280;margin-top:2px">ไม่สามารถใช้งานได้อีกแล้ว</div>
          </div>
          <button onclick="document.getElementById('_rod-shop-overlay').remove()"
            style="width:100%;background:#374151;color:#9ca3af;border:none;
    ...
    <div style="background:linear-gradient(135deg,#0a0f1e,#1a0a2e);border:2.5px solid #fcd34d;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
        <div>
          ...
          <div style="font-size:0.65rem;color:#9ca3af">ใช้ Rod Token แลกการ์ด HxH พิเศษ</div>
        </div>
