...
// ============================================================
        ...
        // ── [FIX] Guard ป้องกัน double-click ────────────────────
        ...
        async function googleLogin() {
            ...
            try {
                // ── ลอง Popup ก่อน (Desktop / Chrome ที่อนุญาต popup) ──
                await firebase.auth().signInWithPopup(provider);
                _authLoginInProgress = false;
            } catch(e) {
                if (e.code === 'auth/popup-blocked' || e.code === 'auth/popup-cancelled-by-user') {
                    // ── Popup ถูกบล็อก → fallback Redirect (Android Chrome) ──
                    console.log('[Auth] Popup blocked, falling back to redirect...');
                    try {
                        await firebase.auth().signInWithRedirect(provider);
                        // หน้าจะ reload — _authLoginInProgress reset อัตโนมัติ
                    } catch(e2) {
                        console.error('Redirect fallback error:', e2.code);
                        if (btn) btn.style.display = 'flex';
                        if (loading) loading.style.display = 'none';
                        if (errEl) { errEl.textContent = 'Login ไม่สำเร็จ: ' + (e2.message || e2.code); errEl.style.display = 'block'; }
                        _authLoginInProgress = false;
                    }
                } else {
                    // error อื่น (network, config ฯลฯ)
