...
// ============================================================
        ...
        // ── [FIX] Guard flags ป้องกัน double-call ──────────────
        let _authLoginInProgress = false;
        let _redirectResultChecked = false;

        // ── [FIX] เรียกจาก initAuth() เพื่อ handle redirect กลับมา ──
        // วางไว้ใน onAuthStateChanged หรือตอน Firebase ready
        async function handleRedirectResult() {
            if (_redirectResultChecked) return;   // เรียกแค่ครั้งเดียว
            _redirectResultChecked = true;
            try {
                const result = await firebase.auth().getRedirectResult();
                if (result && result.user) {
                    console.log('[Auth] Redirect login success:', result.user.displayName);
                    // loadPlayerDataFromFirebase จะถูกเรียกโดย onAuthStateChanged อยู่แล้ว
                }
            } catch(e) {
                // auth/cancelled-popup-request และ auth/null-user ไม่ต้องแสดง error
        ...
        async function googleLogin() {
            // [FIX] ป้องกัน double-click / double-trigger
            ...
            try {
                ...
                // ใช้ signInWithRedirect — Mobile browser ปลอดภัยกว่า signInWithPopup
                await firebase.auth().signInWithRedirect(provider);
                // หน้าจะ redirect ไป Google แล้วกลับมา
                // handleRedirectResult() + onAuthStateChanged จะจัดการต่อ
            } catch(e) {
        ...
        async function googleLogout() {
            ...
            // [FIX] reset flags ตอน logout เพื่อ login ใหม่ได้ปกติ
            _authLoginInProgress = false;
            _redirectResultChecked = false;
