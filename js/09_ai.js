...
// ============================================================
        ...
        function playAI() {
            ...
            if (state.phase === 'MAIN') {
                let maxPlays = 12;
                const playNext = () => {
                    // [FIX] Guard: เช็ค phase/turn ก่อนทุกครั้ง กัน AI ค้างหลัง error
                    ...
                    // [FIX] ถ้า cost หมดหรือมือว่าง → nextPhase ทันที
                    if (ai.cost <= 0 || ai.hand.length === 0) {
                    ...
                    // [FIX] จำ hand size ก่อนเล่น เพื่อตรวจว่าการ์ดถูกเล่นจริงหรือเปล่า
                    const handSizeBefore = ai.hand.length;

                    // [FIX] ครอบ try-catch ทั้งบล็อก กัน AI หยุดถาวรถ้าเกิด error
                    try {
                        ...
                        // [FIX] ถ้า error → nextPhase แทนที่จะ stuck ตลอดกาล
                        console.error('[AI playNext] error playing card:', e);
                        setTimeout(() => nextPhase(), 800);
                        return;
                    }

                    // [FIX] ถ้าการ์ดไม่ถูกเล่น (hand size เท่าเดิม) → หยุดวนซ้ำ card เดิม
                    if (ai.hand.length >= handSizeBefore) {
                        setTimeout(() => nextPhase(), 600);
                        return;
                    }

                    // [FIX] เช็ค phase/turn อีกรอบก่อน schedule ต่อ
                    setTimeout(() => {
                        if (state.phase === 'MAIN' && state.currentTurn === 'ai') playNext();
                    }, 700);
                };
                ...
                const processAIAttack = () => {
                    ...
                    if (!hasBlocker) {
                        // [FIX] try-catch กัน loop ตายถาวรถ้า initiateAttack error
                        try {
                            initiateAttack(attacker.id, null, true);
                        } catch(e) {
                            console.error('[AI processAIAttack base] error:', e);
                            attacker.attacksLeft = 0; // force exhaust กัน infinite loop
                        }
                    ...
                    try {
                        ...
                        console.error('[AI processAIAttack char] error:', e);
                        attacker.attacksLeft = 0; // force exhaust กัน infinite loop
                    }
                    setTimeout(processAIAttack, 700);
                };
