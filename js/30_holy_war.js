...
// ============================================================
// HOTFIX: แก้ไขบัคสเตตัสไอเทมของ Gloxinia (Basquias, Yggdra Armor, Emerald Octo)
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    if (typeof window.getCharStats === 'function') {
        const _origStatsHWItems = window.getCharStats;
        window.getCharStats = function(char) {
            let stats = _origStatsHWItems.apply(this, arguments);
            if (char.silenced) return stats;
            if (char.items && char.items.length > 0) {
                char.items.forEach(item => {
                    ...
                    if (iN === 'Yggdra Armor') {
                        stats.atk += 7;
                        stats.maxHp += 7;
                        if (char.hp > -99) stats.hp = Math.max(0, stats.hp + 7);
                    }
                    if (iN === 'Form Ten: Emerald Octo') {
                        stats.maxHp += 10;
                        if (char.hp > -99) stats.hp = Math.max(0, stats.hp + 10);
                    }
                });
            }
            return stats;
