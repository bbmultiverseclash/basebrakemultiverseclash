...
function _hookHolyWarMechanics() {
    ...
    if (typeof window.initiateAttack === 'function') {
        ...
        window.initiateAttack = function(atkId, tgtId, isBase) {
            ...
            if (attacker && target) {
                ...
                if (aN === 'Dahaaka' && !attacker.silenced && tgtAfter && tgtAfter.hp > 0 && !tgtAfter.tossakanImmune) {
                    if (!tgtAfter.status.includes('Paralyze')) tgtAfter.status.push('Paralyze');
                    tgtAfter.paralyzeTurns = Math.max(tgtAfter.paralyzeTurns || 0, 2);
                    if (typeof log === 'function') log(`🦎 [Dahaaka] พิษอัมพาต! โจมตีทำให้ ${tgtAfter.name} ขยับไม่ได้!`, 'text-lime-400 font-bold');
                }

                if (aN === 'Belion' && !attacker.silenced && (!tgtAfter || tgtAfter.hp <= 0)) {
                    const allies = state.players[atkKey].field.filter(c => c.id !== attacker.id && c.hp > 0);
                    if (allies.length > 0) {
                        const shuffled = [...allies].sort(() => Math.random() - 0.5).slice(0, 2);
                        shuffled.forEach(c => {
                            c.atk += 1; c.maxHp += 1; c.hp += 1;
                            if (typeof log === 'function') log(`🗡️ [Belion] บรรณาการสังหาร! มอบพลังให้ ${c.name} +1/+1`, 'text-slate-400 font-bold');
                        });
                    }
                }
    ...
    if (typeof window.triggerOnSummon === 'function') {
        ...
        window.triggerOnSummon = function(card, pk) {
            ...
            if (eff === 'Monspeet') {
                const enemies = opp.field.filter(c => typeof getCharStats === 'function' && getCharStats(c).hp > 0 && !c.tossakanImmune);
                if (enemies.length > 0) {
                    const target = enemies[Math.floor(Math.random() * enemies.length)];
                    target.silenced = true;
                    target.reticenceTurns = 2;
                    if (typeof log === 'function') log(`🔥 [Monspeet] เปลวเพลิง Reticence! ปิดผนึก ${target.name} (ติดใบ้ 2 เทิร์น)!`, 'text-red-400 font-bold');
                }
            }
            if (eff === 'Chandler') {
    ...
    if (typeof window.resolveEndPhase === 'function') {
        ...
        window.resolveEndPhase = function(pk) {
            ...
            p.field.forEach(c => {
                ...
                // ลดเทิร์นการใบ้ของ Monspeet (Reticence)
                if (c.reticenceTurns > 0) {
                    c.reticenceTurns--;
                    if (c.reticenceTurns <= 0) {
                        c.silenced = false;
                        if (typeof log === 'function') log(`🔥[Reticence] อาคมของ Monspeet คลายลง ${c.name} กลับมาใช้สกิลได้`, 'text-gray-400');
                    }
                }

                // Derieri: ถ้ามีสถานะผิดปกติหุ้มตัว ให้ +3 HP
                if (eff === 'Derieri' && c.status && c.status.length > 0 && c.hp > 0) {
                    c.maxHp += 3;
                    c.hp += 3;
                    if (typeof log === 'function') log(`💥 [Derieri] ความมืดเกาะกินร่าง! เปลี่ยนสถานะเป็นพลัง +3 HP`, 'text-orange-400 font-bold');
                }
...
// ============================================================
// 38_gloxinia_fix.js — แก้บัคสเตตัสไอเทมของ Gloxinia (Direct Stat Approach)
// ============================================================
document.addEventListener('DOMContentLoaded', () => {
    // 1. เขียนทับ executeGloxiniaForm ให้บวกค่าพลังโดยตรงเลย
    window.executeGloxiniaForm = function(glox, pk) {
        const p = state.players[pk];
        const rand = Math.floor(Math.random() * 6);
        const _HW_THEME = 'holy_war';
        const _HW_TOKENS = {
            'Basquias':               { name: 'Basquias',               type: 'Item',  cost: 0, text: '+2 ATK | โจมตี: 2 ดาเมจใส่การ์ดศัตรูสุ่ม 1 ใบ',                                                          color: 'bg-yellow-600',  requiresTarget: true  },
            'Form Two: Guardian':     { name: 'Form Two: Guardian',     type: 'Spell', cost: 5, text: 'เรียก Guardian 5/5 (Taunt) หากมี Gloxinia จะเป็น ATK 10',                                               color: 'bg-lime-700',    requiresTarget: false },
            'Yggdra Armor':           { name: 'Yggdra Armor',           type: 'Item',  cost: 0, text: '+7 ATK / +7 HP | เมื่อผู้สวมใส่ตาย ทำลายไอเทมนี้ ผู้ใส่รอดด้วย HP=1 และ Immortal 1 เทิร์น',            color: 'bg-emerald-600', requiresTarget: true  },
            'Form Seven: Moon Rose':  { name: 'Form Seven: Moon Rose',  type: 'Spell', cost: 1, text: 'ฮีล 2 HP ให้ 1 เป้าหมาย | หากเป็น Drole หรือ Gloxinia จะฮีลเต็มแทน',                                   color: 'bg-pink-500',    requiresTarget: true, targetEnemy: false },
            'Form Nine: Death Thorn': { name: 'Form Nine: Death Thorn', type: 'Spell', cost: 5, text: 'ทำ 10 ดาเมจ 10 ครั้ง สุ่มเป้าหมาย (โอกาสสำเร็จครั้งละ 10%)',                                           color: 'bg-purple-800',  requiresTarget: false },
            'Form Ten: Emerald Octo': { name: 'Form Ten: Emerald Octo', type: 'Item',  cost: 0, text: '+10 HP | โจมตี: เป้าหมายติด Paralyze',                                                                   color: 'bg-teal-600',    requiresTarget: true  }
        };
        const mkSp = (n) => {
            const db = _HW_TOKENS[n];
            return { id: 'card_'+Date.now(), name: db.name, originalName: db.name, type: db.type, cost: db.cost, text: db.text, color: db.color, _theme: _HW_THEME, requiresTarget: db.requiresTarget };
        };

        if (rand === 0) {
            glox.items.push(mkSp('Basquias'));
            glox.atk += 2;
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 1: Basquias! (+2 ATK)`, 'text-green-300 font-bold');
        } else if (rand === 1) {
            p.hand.push(mkSp('Form Two: Guardian'));
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 2: Guardian!`, 'text-green-300 font-bold');
        } else if (rand === 2) {
            glox.items.push(mkSp('Yggdra Armor'));
            glox.atk += 7; glox.maxHp += 7; glox.hp += 7;
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 5: Yggdra Armor! (+7 ATK/+7 HP)`, 'text-green-300 font-bold');
        } else if (rand === 3) {
            p.hand.push(mkSp('Form Seven: Moon Rose'));
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 7: Moon Rose!`, 'text-green-300 font-bold');
        } else if (rand === 4) {
            p.hand.push(mkSp('Form Nine: Death Thorn'));
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 9: Death Thorn!`, 'text-green-300 font-bold');
        } else if (rand === 5) {
            glox.items.push(mkSp('Form Ten: Emerald Octo'));
            glox.maxHp += 10; glox.hp += 10;
            if (typeof log === 'function') log(`🧚 [Gloxinia] Form 10: Emerald Octo! (+10 HP)`, 'text-green-300 font-bold');
        }

        if (typeof updateUI === 'function') updateUI();
    };

    // 2. ดัก checkDeath เพื่อหัก stat Yggdra Armor ออกตอนเกราะแตก
    if (typeof window.checkDeath === 'function' && !window._gloxYggdraDeathPatched) {
        const _origCheckDeathGloxFix = window.checkDeath;
        window.checkDeath = function(pk) {
            const p = state.players[pk];
            p.field.forEach(c => {
                if (c.hp <= 0 && !c.isDyingProcessing) {
                    if (c.items && c.items.some(i => i.name === 'Yggdra Armor')) {
                        c.hp = 1; c.immortalTurns = 2;
                        c.items = c.items.filter(i => i.name !== 'Yggdra Armor');
                        c.atk = Math.max(0, c.atk - 7);
                        c.maxHp = Math.max(1, c.maxHp - 7);
                        c.isDyingProcessing = false;
                        if (typeof log === 'function') log(`🛡️ เกราะ Yggdra ของ ${c.name} แตกสลาย! ช่วยชีวิตไว้ได้! (-7 ATK/HP)`, 'text-emerald-400 font-bold');
                        return;
                    }
                }
            });
            _origCheckDeathGloxFix.apply(this, arguments);
        };
        window._gloxYggdraDeathPatched = true;
    }
});
