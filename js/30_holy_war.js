...
window.startDungeon_hw_demons = function() {
    ...
    const deckCards = (deck.cards || []).map(c => typeof c === 'string' ? { name: c, theme: deck.theme || HW_THEME } : c);
    if (typeof window !== 'undefined') {
        window._pendingCollectionDeck = deckCards;
        window._collectionDeckUsed = false;
        window._isRankedGame = false;
    ...
    // Override buildDeck สำหรับฝั่ง AI เป็นกองทัพปีศาจบัฟ
    const _origBuildDeckHW = window.buildDeck;
    window.buildDeck = function(theme) {
        if (theme === 'hw_demons_boss') {
            const bossDeck = [];
            const makeDemon = (name, atk, hp) => {
                const c = typeof createCardInstance === 'function' ? createCardInstance(name, HW_THEME) : null;
                if (c) {
                    c.atk = atk; c.hp = hp; c.maxHp = hp;
                    c.isDungeonBoss2 = true; // ป้องกัน Instant Kill บางประเภท
                }
                return c;
            };
            // Red Demon 10/10 (15 ใบ)
            for (let i = 0; i < 15; i++) bossDeck.push(makeDemon('Red Demon', 10, 10));
            // Grey Demon 15/15 (15 ใบ)
            for (let i = 0; i < 15; i++) bossDeck.push(makeDemon('Grey Demon', 15, 15));
            // White Demon 5/5 (20 ใบ)
            for (let i = 0; i < 20; i++) bossDeck.push(makeDemon('White Demon', 5, 5));
            // Silver Demon 20/20 (10 ใบ)
    ...
    if (typeof selectedPlayerTheme !== 'undefined') window.selectedPlayerTheme = deck.theme || HW_THEME;
    if (typeof selectedAITheme !== 'undefined') window.selectedAITheme = 'hw_demons_boss';
    // ใช้ global variable ตรงๆ ถ้า selectedPlayerTheme/selectedAITheme เป็น global
    selectedPlayerTheme = deck.theme || HW_THEME;
