...
// ============================================================
    ...
    checkDeath(playerKey);
                ...
                log(`✦━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━✦`, 'text-yellow-500 font-bold');
                log(`♛ [Altair] Champion ลงสนาม! ATK 12 / HP 10 | On Attack: ชุบเพื่อน | On Death: ภูมิคุ้มกันสะสม`, 'text-yellow-400 font-bold');
                // ── Cost Reduction Permanent (สะสมข้ามเกม) ──
                if (typeof playerData !== 'undefined') {
                    playerData.altairCostBonus = (playerData.altairCostBonus || 0) + 1;
                    card.costReducer = (card.costReducer || 0) + 1;
                    saveData();
                    const currentCost = Math.max(0, 12 - (playerData.altairCostBonus || 0));
                    log(`♛ [Altair] Cost ลดถาวร! ตอนนี้ Cost ${currentCost} (ลดสะสมไปแล้ว ${playerData.altairCostBonus} ครั้ง)`, 'text-yellow-300 font-bold');
                }
            }
            ...
            if (effectiveName === 'Luis Pasteur') {
                ...
                const allies = p.field.filter(cc => cc.type === 'Character' && cc.id !== card.id && getCharStats(cc).hp > 0);
                if (allies.length > 0) {
                    const target = allies[Math.floor(Math.random() * allies.length)];
                    target.hp = getCharStats(target).maxHp;
                    log(`[Pasteur] ฮีลเต็ม HP ให้ ${target.name}`, 'text-indigo-300 font-bold');
                } else {
                    log(`[Pasteur] ไม่มีเพื่อนให้ฮีล`, 'text-gray-500');
                }
            }
