...
// ============================================================
        ...
        function buildDeck(theme) {
            ...
            if (theme === "isekai_adventure") {
                ...
                // ── Altair Champion ──
                if (typeof playerData !== 'undefined' && (playerData.altairUnlocked || (playerData.collection && playerData.collection['Altair|isekai_adventure']))) {
                    const altairCard = createCardInstance('Altair', theme);
                    altairCard.costReducer = 0; // เริ่มต้นที่เกมใหม่ Cost จะยังไม่ถูกลด
                    deck.push(altairCard);
                }
            } 
...
function tickTempBuffs(playerKey) {
    ...
    p.field.forEach(c => {
        ...
        function getCharStats(char) {
            ...
            if (effectiveName === 'One Eye' && !char.silenced) {
                totalAtk += spaceZoneCount;
            }
            if (effectiveName === 'Galax Dragon' && !char.silenced) {
                if (spaceZoneCount >= 7) {
                    totalHp += 5;
                    totalMaxHp += 5;
                }
            ...
            // ── Regulus Corneas: อมตะถ้ามีการ์ดที่ถูก Mark อยู่ในสนาม ──────
            if (effectiveName === 'Regulus Corneas') {
                const hasMark = ownField.some(c => c._regulusMarked && c.hp > 0);
                if (hasMark) damageMultiplier = 0; // รับดาเมจ 0 (อมตะ)
            }
    ...
    // Cecilus Segmunt: ถ้ามี Cecilus บนสนาม → Murasame/Masayume Cost ลดตาม HP ที่สูญเสีย
    if (card.name === 'Murasame' || card.name === 'Masayume') {
        const hasCecilus = state.players[playerKey].field.some(c => {
            const n = (c.name.startsWith('Shadow Token') || c.name.startsWith('Shadow army') || c.name.includes('Loki Clone')) ? c.originalName : c.name;
            return n === 'Cecilus Segmunt' && c.hp > 0;
        });
        if (hasCecilus) {
            const hpLost = 20 - state.players[playerKey].hp;
            baseCost = Math.max(0, baseCost - hpLost);
        }
    }
    ...
    return Math.max(0, baseCost - (card.costReducer || 0));
        ...
        function nextPhase() {
            // Online/Draft P2: ส่ง action ให้ host execute (พร้อม fromPhase)
            if ((gameMode === 'online' || gameMode === 'draft') && myRole === 'ai' && state.currentTurn === 'ai') {
                sendOnlineAction({ type: 'nextPhase', fromPhase: state.phase }); return;
            }
                        ...
                        if (item.name === "Genie's Lamp") {
                            ...
                            if (item.wishCount < 3) {
                                ...
                                if (wishRoll === 0) {
                                    ...
                                    log(`🧞 [Genie's Lamp] Wish #${item.wishCount}: 40% Damage Reduction! ${c.name}!`, 'text-yellow-400 font-bold');
                                }
